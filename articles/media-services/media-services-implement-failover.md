...
---
# <a name="implement-failover-streaming-with-azure-media-services"></a><span data-ttu-id="d7913-103">Failover streamen met Azure Media Services implementeren</span><span class="sxs-lookup"><span data-stu-id="d7913-103">Implement failover streaming with Azure Media Services</span></span>

<span data-ttu-id="d7913-104">In dit scenario laat zien hoe toocopy inhoud (BLOB's) van het ene activum in een ander in volgorde toohandle redundantie voor streaming on demand.</span><span class="sxs-lookup"><span data-stu-id="d7913-104">This walkthrough demonstrates how toocopy content (blobs) from one asset into another in order toohandle redundancy for on-demand streaming.</span></span> <span data-ttu-id="d7913-105">Dit scenario is nuttig als u tooset van Azure Content Delivery Network toofail via tussen twee datacentra, in geval van een storing in een datacenter.</span><span class="sxs-lookup"><span data-stu-id="d7913-105">This scenario is useful if you want tooset up Azure Content Delivery Network toofail over between two datacenters, in case of an outage in one datacenter.</span></span> <span data-ttu-id="d7913-106">In dit scenario gebruikt hello Azure Media Services SDK, Hallo REST-API van Azure Media Services en hello Azure-opslag-SDK toodemonstrate Hallo taken te volgen:</span><span class="sxs-lookup"><span data-stu-id="d7913-106">This walkthrough uses hello Azure Media Services SDK, hello Azure Media Services REST API, and hello Azure Storage SDK toodemonstrate hello following tasks:</span></span>

1. <span data-ttu-id="d7913-107">Een Media Services-account in 'Data Center-a' instellen</span><span class="sxs-lookup"><span data-stu-id="d7913-107">Set up a Media Services account in "Data Center A."</span></span>
2. <span data-ttu-id="d7913-108">Upload een tussentijds bestand naar een activum bron.</span><span class="sxs-lookup"><span data-stu-id="d7913-108">Upload a mezzanine file into a source asset.</span></span>
3. <span data-ttu-id="d7913-109">Hallo asset coderen in meerdere bit snelheid MP4-bestanden.</span><span class="sxs-lookup"><span data-stu-id="d7913-109">Encode hello asset into multi-bit rate MP4 files.</span></span> 
4. <span data-ttu-id="d7913-110">Een alleen-lezen shared access signature locator maken.</span><span class="sxs-lookup"><span data-stu-id="d7913-110">Create a read-only shared access signature locator.</span></span> <span data-ttu-id="d7913-111">Dit is voor Hallo bron asset toohave leestoegang toohello container in Hallo storage-account dat is gekoppeld aan Hallo bron asset.</span><span class="sxs-lookup"><span data-stu-id="d7913-111">This is for hello source asset toohave read access toohello container in hello storage account that is associated with hello source asset.</span></span>
5. <span data-ttu-id="d7913-112">De containernaam Hallo van Hallo bron actief ophalen van Hallo alleen-lezen toegang van gedeelde handtekening locator gemaakt in de vorige stap Hallo.</span><span class="sxs-lookup"><span data-stu-id="d7913-112">Get hello container name of hello source asset from hello read-only shared access signature locator created in hello previous step.</span></span> <span data-ttu-id="d7913-113">Dit is nodig voor het kopiëren van BLOB's tussen opslagaccounts (dit wordt verderop in Hallo onderwerp.)</span><span class="sxs-lookup"><span data-stu-id="d7913-113">This is necessary for copying blobs between storage accounts (explained later in hello topic.)</span></span>
6. <span data-ttu-id="d7913-114">Een locator oorsprong voor Hallo activa dat is gemaakt door Hallo codering taak maken.</span><span class="sxs-lookup"><span data-stu-id="d7913-114">Create an origin locator for hello asset that was created by hello encoding task.</span></span> 

<span data-ttu-id="d7913-115">Vervolgens toohandle Hallo failover:</span><span class="sxs-lookup"><span data-stu-id="d7913-115">Then, toohandle hello failover:</span></span>

1. <span data-ttu-id="d7913-116">Een Media Services-account in 'Data Center B.' instellen</span><span class="sxs-lookup"><span data-stu-id="d7913-116">Set up a Media Services account in "Data Center B."</span></span>
2. <span data-ttu-id="d7913-117">Een leeg doel-activum maken in Hallo doel Media Services-account.</span><span class="sxs-lookup"><span data-stu-id="d7913-117">Create a target empty asset in hello target Media Services account.</span></span>
3. <span data-ttu-id="d7913-118">Maak een locator schrijven gedeeld toegang handtekening.</span><span class="sxs-lookup"><span data-stu-id="d7913-118">Create a write shared access signature locator.</span></span> <span data-ttu-id="d7913-119">Dit is voor Hallo leeg asset toohave schrijftoegang toohello doelcontainer in Hallo doel storage-account dat is gekoppeld aan Hallo doel asset.</span><span class="sxs-lookup"><span data-stu-id="d7913-119">This is for hello target empty asset toohave write access toohello container in hello target storage account that is associated with hello target asset.</span></span>
4. <span data-ttu-id="d7913-120">Hello Azure-opslag-SDK toocopy BLOB's (assetbestanden) gebruiken tussen Hallo bron storage-account in "Data Center A" en doelopslagaccount in 'Data Center B.' Hallo</span><span class="sxs-lookup"><span data-stu-id="d7913-120">Use hello Azure Storage SDK toocopy blobs (asset files) between hello source storage account in "Data Center A" and hello target storage account in "Data Center B."</span></span> <span data-ttu-id="d7913-121">Deze storage-accounts zijn gekoppeld aan Hallo activa van belang.</span><span class="sxs-lookup"><span data-stu-id="d7913-121">These storage accounts are associated with hello assets of interest.</span></span>
5. <span data-ttu-id="d7913-122">Blobs (assetbestanden) die gekopieerd toohello doel blob-container met Hallo doel asset zijn koppelen.</span><span class="sxs-lookup"><span data-stu-id="d7913-122">Associate blobs (asset files) that were copied toohello target blob container with hello target asset.</span></span> 
6. <span data-ttu-id="d7913-123">Maken van een oorsprong-locator voor de activa in 'Data Center-B' Hallo en Hallo locator-ID die is gegenereerd voor de activa in 'Data Center-a' hello opgeven</span><span class="sxs-lookup"><span data-stu-id="d7913-123">Create an origin locator for hello asset in "Data Center B", and specify hello locator ID that was generated for hello asset in "Data Center A."</span></span>

<span data-ttu-id="d7913-124">Dit biedt u streaming URL's waar Hallo relatieve paden van Hallo URL's zijn Hallo Hallo dezelfde (alleen hello basis-URL's zijn verschillende).</span><span class="sxs-lookup"><span data-stu-id="d7913-124">This gives you hello streaming URLs where hello relative paths of hello URLs are hello same (only hello base URLs are different).</span></span> 

<span data-ttu-id="d7913-125">Vervolgens toohandle eventuele uitval, kunt u een Content Delivery Network boven op de locators voor deze oorsprong.</span><span class="sxs-lookup"><span data-stu-id="d7913-125">Then, toohandle any outages, you can create a Content Delivery Network on top of these origin locators.</span></span> 

<span data-ttu-id="d7913-126">Hallo overwegingen volgende van toepassing:</span><span class="sxs-lookup"><span data-stu-id="d7913-126">hello following considerations apply:</span></span>

* <span data-ttu-id="d7913-127">de huidige versie Hallo van Media Services SDK biedt geen ondersteuning voor programmatisch genereren IAssetFile-informatie die u aan een asset assetbestanden koppelen wilt.</span><span class="sxs-lookup"><span data-stu-id="d7913-127">hello current version of Media Services SDK does not support programmatically generating IAssetFile information that would associate an asset with asset files.</span></span> <span data-ttu-id="d7913-128">In plaats daarvan Hallo CreateFileInfos Media Services REST-API toodo deze gebruiken.</span><span class="sxs-lookup"><span data-stu-id="d7913-128">Instead, use hello CreateFileInfos Media Services REST API toodo this.</span></span> 
* <span data-ttu-id="d7913-129">Opslag versleuteld activa (AssetCreationOptions.StorageEncrypted) worden niet ondersteund voor replicatie (omdat de versleutelingssleutel Hallo verschilt voor beide Media Services-accounts).</span><span class="sxs-lookup"><span data-stu-id="d7913-129">Storage encrypted assets (AssetCreationOptions.StorageEncrypted) are not supported for replication (because hello encryption key is different in both Media Services accounts).</span></span> 
* <span data-ttu-id="d7913-130">Als u tootake profiteren van dynamische pakketten wilt, controleert u of Hallo streaming-eindpunt van waaruit u wilt dat toostream uw inhoud in Hallo **met** status.</span><span class="sxs-lookup"><span data-stu-id="d7913-130">If you want tootake advantage of dynamic packaging, make sure hello streaming endpoint from which you want toostream  your content is in hello **Running** state.</span></span>

> [!NOTE]
> <span data-ttu-id="d7913-131">Overweeg het gebruik van Hallo Media Services [Replicator hulpprogramma](http://replicator.codeplex.com/) als een alternatieve tooimplementing een failover streaming scenario handmatig.</span><span class="sxs-lookup"><span data-stu-id="d7913-131">Consider using hello Media Services [Replicator Tool](http://replicator.codeplex.com/) as an alternative tooimplementing a failover streaming scenario manually.</span></span> <span data-ttu-id="d7913-132">Dit hulpprogramma kunt u tooreplicate activa tussen twee Media Services-accounts.</span><span class="sxs-lookup"><span data-stu-id="d7913-132">This tool allows you tooreplicate assets across two Media Services accounts.</span></span>
> 
> 

## <a name="prerequisites"></a><span data-ttu-id="d7913-133">Vereisten</span><span class="sxs-lookup"><span data-stu-id="d7913-133">Prerequisites</span></span>
* <span data-ttu-id="d7913-134">Twee Media Services-accounts in een nieuw of bestaand Azure-abonnement.</span><span class="sxs-lookup"><span data-stu-id="d7913-134">Two Media Services accounts in a new or existing Azure subscription.</span></span> <span data-ttu-id="d7913-135">Zie [hoe tooCreate een Media Services-Account](media-services-portal-create-account.md).</span><span class="sxs-lookup"><span data-stu-id="d7913-135">See [How tooCreate a Media Services Account](media-services-portal-create-account.md).</span></span>
* <span data-ttu-id="d7913-136">Besturingssysteem: Windows 7, Windows 2008 R2 of Windows 8.</span><span class="sxs-lookup"><span data-stu-id="d7913-136">Operating system: Windows 7, Windows 2008 R2, or Windows 8.</span></span>
* <span data-ttu-id="d7913-137">.NET framework 4.5 of .NET Framework 4.</span><span class="sxs-lookup"><span data-stu-id="d7913-137">.NET Framework 4.5 or .NET Framework 4.</span></span>
* <span data-ttu-id="d7913-138">Visual Studio 2010 SP1 of hoger (Professional, Premium, Ultimate of Express).</span><span class="sxs-lookup"><span data-stu-id="d7913-138">Visual Studio 2010 SP1 or later version (Professional, Premium, Ultimate, or Express).</span></span>

## <a name="set-up-your-project"></a><span data-ttu-id="d7913-139">Instellen van uw project</span><span class="sxs-lookup"><span data-stu-id="d7913-139">Set up your project</span></span>
<span data-ttu-id="d7913-140">In deze sectie maken en instellen van een project C#-consoletoepassing.</span><span class="sxs-lookup"><span data-stu-id="d7913-140">In this section, you create and set up a C# Console Application project.</span></span>

1. <span data-ttu-id="d7913-141">Gebruik Visual Studio toocreate een nieuwe oplossing waarin Hallo C#-consoletoepassing project.</span><span class="sxs-lookup"><span data-stu-id="d7913-141">Use Visual Studio toocreate a new solution that contains hello C# Console Application project.</span></span> <span data-ttu-id="d7913-142">Voer **HandleRedundancyForOnDemandStreaming** voor Hallo naam en klik vervolgens op **OK**.</span><span class="sxs-lookup"><span data-stu-id="d7913-142">Enter **HandleRedundancyForOnDemandStreaming** for hello name, and then click **OK**.</span></span>
2. <span data-ttu-id="d7913-143">Hallo maken **SupportFiles** map op Hallo van hetzelfde niveau als Hallo **HandleRedundancyForOnDemandStreaming.csproj** projectbestand.</span><span class="sxs-lookup"><span data-stu-id="d7913-143">Create hello **SupportFiles** folder on hello same level as hello **HandleRedundancyForOnDemandStreaming.csproj** project file.</span></span> <span data-ttu-id="d7913-144">Onder Hallo **SupportFiles** map maken Hallo **OutputFiles** en **MP4Files** mappen.</span><span class="sxs-lookup"><span data-stu-id="d7913-144">Under hello **SupportFiles** folder, create hello **OutputFiles** and **MP4Files** folders.</span></span> <span data-ttu-id="d7913-145">Een MP4-bestand kopiëren naar Hallo **MP4Files** map.</span><span class="sxs-lookup"><span data-stu-id="d7913-145">Copy an .mp4 file into hello **MP4Files** folder.</span></span> <span data-ttu-id="d7913-146">(In dit voorbeeld Hallo **BigBuckBunny.mp4** -bestand wordt gebruikt.)</span><span class="sxs-lookup"><span data-stu-id="d7913-146">(In this example, hello **BigBuckBunny.mp4** file is used.)</span></span> 
3. <span data-ttu-id="d7913-147">Gebruik **Nuget** tooadd verwijzingen tooDLLs gerelateerde tooMedia Services.</span><span class="sxs-lookup"><span data-stu-id="d7913-147">Use **Nuget** tooadd references tooDLLs related tooMedia Services.</span></span> <span data-ttu-id="d7913-148">In **Visual Studio in het hoofdmenu**, selecteer **extra** > **Library Package Manager** > **Package Manager Console**.</span><span class="sxs-lookup"><span data-stu-id="d7913-148">In **Visual Studio Main Menu**, select **TOOLS** > **Library Package Manager** > **Package Manager Console**.</span></span> <span data-ttu-id="d7913-149">Typ in het consolevenster hello, **Install-Package windowsazure.mediaservices**, en druk op Enter.</span><span class="sxs-lookup"><span data-stu-id="d7913-149">In hello console window, type **Install-Package windowsazure.mediaservices**, and press Enter.</span></span>
4. <span data-ttu-id="d7913-150">Voeg andere verwijzingen die vereist voor dit project zijn toe: System.Configuration System.Runtime.Serialization en System.Web.</span><span class="sxs-lookup"><span data-stu-id="d7913-150">Add other references that are required for this project: System.Configuration, System.Runtime.Serialization, and System.Web.</span></span>
5. <span data-ttu-id="d7913-151">Vervang **met** instructies die zijn toegevoegd toohello **Programs.cs** bestand met de volgende toepassingsgroepen Hallo standaard:</span><span class="sxs-lookup"><span data-stu-id="d7913-151">Replace **using** statements that were added toohello **Programs.cs** file by default with hello following ones:</span></span>
...
6. <span data-ttu-id="d7913-152">Hallo toevoegen **appSettings** sectie toohello **.config** bestands- en update Hallo waarden op basis van uw Media Services en opslag en de naam waarden.</span><span class="sxs-lookup"><span data-stu-id="d7913-152">Add hello **appSettings** section toohello **.config** file, and update hello values based on your Media Services and Storage key and name values.</span></span> 
...
## <a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a><span data-ttu-id="d7913-153">Code die verantwoordelijk is voor redundantie voor streaming on demand toevoegen</span><span class="sxs-lookup"><span data-stu-id="d7913-153">Add code that handles redundancy for on-demand streaming</span></span>
<span data-ttu-id="d7913-154">In deze sectie maakt u Hallo mogelijkheid toohandle redundantie.</span><span class="sxs-lookup"><span data-stu-id="d7913-154">In this section, you create hello ability toohandle redundancy.</span></span>

1. <span data-ttu-id="d7913-155">Hallo na klasseniveau velden toohello programma klasse toevoegen.</span><span class="sxs-lookup"><span data-stu-id="d7913-155">Add hello following class-level fields toohello Program class.</span></span>
...
2. <span data-ttu-id="d7913-156">Hallo standaard Main-methodedefinitie vervangen door Hallo volgt.</span><span class="sxs-lookup"><span data-stu-id="d7913-156">Replace hello default Main method definition with hello following one.</span></span> <span data-ttu-id="d7913-157">Methode-netwerkdefinities die worden aangeroepen vanuit Main worden hieronder gedefinieerd.</span><span class="sxs-lookup"><span data-stu-id="d7913-157">Method definitions that are called from Main are defined below.</span></span>
...
3. <span data-ttu-id="d7913-158">Hallo volgende methodedefinities worden aangeroepen vanuit Main.</span><span class="sxs-lookup"><span data-stu-id="d7913-158">hello following method definitions are called from Main.</span></span>

    >[!NOTE]
    ><span data-ttu-id="d7913-159">Er is een limiet van 1.000.000 beleidsregels voor verschillende Media Services-beleidsregels (bijvoorbeeld voor Locator beleid of ContentKeyAuthorizationPolicy).</span><span class="sxs-lookup"><span data-stu-id="d7913-159">There is a limit of 1,000,000 policies for different Media Services policies (for example, for Locator policy or ContentKeyAuthorizationPolicy).</span></span> <span data-ttu-id="d7913-160">Hallo moet u dezelfde beleids-ID als u altijd dezelfde dagen en toegang Hallo machtigingen.</span><span class="sxs-lookup"><span data-stu-id="d7913-160">You should use hello same policy ID if you are always using hello same days and access permissions.</span></span> <span data-ttu-id="d7913-161">Bijvoorbeeld Hallo dezelfde ID gebruiken voor beleid voor locators die beoogde tooremain aanwezig gedurende een lange periode (niet-upload policies zijn).</span><span class="sxs-lookup"><span data-stu-id="d7913-161">For example, use hello same ID for policies for locators that are intended tooremain in place for a long time (non-upload policies).</span></span> <span data-ttu-id="d7913-162">Zie voor meer informatie [in dit onderwerp](media-services-dotnet-manage-entities.md#limit-access-policies).</span><span class="sxs-lookup"><span data-stu-id="d7913-162">For more information, see [this topic](media-services-dotnet-manage-entities.md#limit-access-policies).</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="d7913-163">Volgende stappen</span><span class="sxs-lookup"><span data-stu-id="d7913-163">Next steps</span></span>
<span data-ttu-id="d7913-164">U kunt nu een traffic manager tooroute aanvragen tussen twee datacentra hello gebruiken en dus failover in geval van eventuele uitval.</span><span class="sxs-lookup"><span data-stu-id="d7913-164">You can now use a traffic manager tooroute requests between hello two datacenters, and thus fail over in case of any outages.</span></span>

## <a name="media-services-learning-paths"></a><span data-ttu-id="d7913-165">Media Services-leertrajecten</span><span class="sxs-lookup"><span data-stu-id="d7913-165">Media Services learning paths</span></span>
[!INCLUDE [media-services-learning-paths-include](../../includes/media-services-learning-paths-include.md)]

## <a name="provide-feedback"></a><span data-ttu-id="d7913-166">Feedback geven</span><span class="sxs-lookup"><span data-stu-id="d7913-166">Provide feedback</span></span>
