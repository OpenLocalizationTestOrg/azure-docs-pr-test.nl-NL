...
Een codeertaak is gekoppeld aan een invoer asset (of activa) waarop tooperform sommige taken codering. Bijvoorbeeld coderen een MP4-bestand tooH.264 MP4 adaptive bitrate ingesteld; Maak een miniatuur; overlays maken. Na voltooiing van een taak wordt een uitvoerasset geproduceerd.  Hallo uitvoerasset bevat video, audio, miniaturen, enz. Hallo uitvoerasset bevat ook een bestand met metagegevens over Hallo uitvoerasset. Hallo naam van XML-bestand met metagegevens Hallo heeft Hallo volgende indeling: &lt;source_file_name&gt;_manifest.xml (bijvoorbeeld BigBuckBunny_manifest.xml).  

Als u tooexamine Hallo metagegevensbestand wilt, kunt u een **SAS** locator en downloaden Hallo bestand tooyour lokale computer.  

Dit onderwerp wordt besproken Hallo-elementen en typen van Hallo XML-schema op welke Hallo uitvoer metada (&lt;source_file_name&gt;_manifest.xml) is gebaseerd. Zie voor meer informatie over het Hallo-bestand met metagegevens over Hallo invoer asset [invoer metagegevens](media-services-input-metadata-schema.md).  

> [!NOTE]
> U vindt Hallo volledig schema code en XML-voorbeeld aan Hallo einde van dit onderwerp.  
>
>

## <a name="AssetFiles "></a>Het hoofdelement AssetFiles
Verzameling AssetFile vermeldingen voor Hallo coderingstaak.  

### <a name="child-elements"></a>Onderliggende elementen
| Naam | Beschrijving |
| --- | --- |
| **AssetFile**<br/><br/> minOccurs = "0" maxOccurs = '1' |Een [AssetFile element](media-services-output-metadata-schema.md) dat onderdeel is van Hallo AssetFiles verzameling. |
...
| --- | --- | --- |
| **Naam**<br/><br/> Vereist |**xs:String** |Hallo media asset-bestandsnaam. |
| **Grootte**<br/><br/> minInclusive = '0'<br/><br/> Vereist |**xs:Long** |Grootte van Hallo asset bestandsgrootte in bytes. |
| **Duur**<br/><br/> Vereist |**xs: duration** |Inhoud afspelen back duur. |

### <a name="child-elements"></a>Onderliggende elementen
| Naam | Beschrijving |
| --- | --- |
| **Bronnen** |Verzameling van de invoerbron/mediabestanden, die is verwerkt in volgorde tooproduce deze AssetFile. Zie voor meer informatie [bronelement](media-services-output-metadata-schema.md). |
| **VideoTracks**<br/><br/> minOccurs = "0" maxOccurs = '1' |Elke fysieke AssetFile kan nul of meer video houdt interleaved naar een indeling voor de juiste container in het bevatten. Dit is de verzameling Hallo van deze video houdt. Zie voor meer informatie [VideoTracks element](media-services-output-metadata-schema.md). |
| **AudioTracks**<br/><br/> minOccurs = "0" maxOccurs = '1' |Elke fysieke AssetFile kan nul of meer audio houdt interleaved naar een indeling voor de juiste container in het bevatten. Dit is de audio houdt Hallo verzameling. Zie voor meer informatie [AudioTracks element](media-services-output-metadata-schema.md). |

## <a name="Sources "></a>Bronnen-element
Verzameling van de invoerbron/mediabestanden, die is verwerkt in volgorde tooproduce deze AssetFile.  
...
Elke fysieke AssetFile kan nul of meer video houdt interleaved naar een indeling voor de juiste container in het bevatten. Dit is de verzameling Hallo van deze video houdt.  
...
| Naam | Beschrijving |
| --- | --- |
| **VideoTrack**<br/><br/> minOccurs = "1" maxOccurs = 'unbounded' |Een specifieke video in Hallo bovenliggende AssetFile bijhouden. Zie voor meer informatie [VideoTrack element](media-services-output-metadata-schema.md#VideoTrack). |

## <a name="VideoTrack"></a>VideoTrack element
Een specifieke video in Hallo bovenliggende AssetFile bijhouden.  
...
| Naam | Type | Beschrijving |
| --- | --- | --- |
| **ID**<br/><br/> minInclusive = '0'<br/><br/> Vereist |**xs:int** |Op nul gebaseerde index van deze video bijhouden. **Opmerking:** dit is niet noodzakelijkerwijs Hallo TrackID zoals gebruikt in een MP4-bestand. |
| **Code**<br/><br/> Vereist |**xs:String** |Video-codec code code. |
| **Profiel** |**xs:String** |Profiel van de geselecteerde instelling H264 (alleen van toepassing tooH264 codec). |
| **Niveau** |**xs:String** |Geselecteerde instelling H264 niveau (alleen van toepassing tooH264 codec). |
...
| **Bitrate**<br/><br/> minInclusive = '0'<br/><br/> Vereist |**xs:int** |Gemiddelde video bitsnelheid in kilobits per seconde berekend op basis van Hallo AssetFile. Alleen Hallo elementaire stroom nettolading telt en bevat geen Hallo verpakking overhead. |
| **TargetBitrate**<br/><br/> minInclusive = '0'<br/><br/> Vereist |**xs:int** |Doel gemiddelde bitrate voor deze video bijhouden, zoals aangevraagd via de codering voorinstelling Hallo in kilobits per seconde. |
...
Elke fysieke AssetFile kan nul of meer audio houdt interleaved naar een indeling voor de juiste container in het bevatten. Dit is de audio houdt Hallo verzameling.  
...
| Naam | Beschrijving |
| --- | --- |
| **AudioTrack**<br/><br/> minOccurs = "1" maxOccurs = 'unbounded' |Een specifieke audio in Hallo bovenliggende AssetFile bijhouden. Zie voor meer informatie [AudioTrack element](media-services-output-metadata-schema.md). |

## <a name="AudioTrack "></a>AudioTrack element
Een specifieke audio in Hallo bovenliggende AssetFile bijhouden.  
...
| Naam | Type | Beschrijving |
| --- | --- | --- |
| **ID**<br/><br/> minInclusive = '0'<br/><br/> Vereist |**xs:int** |Op nul gebaseerde index van deze-nummer. **Opmerking:** dit is niet noodzakelijkerwijs Hallo TrackID zoals gebruikt in een MP4-bestand. |
...
| **Bitrate**<br/><br/> minInclusive = '0'<br/><br/> Vereist |**xs:int** |Gemiddelde audio bitsnelheid in bits per seconde berekend op basis van Hallo AssetFile. Alleen Hallo elementaire stroom nettolading telt en bevat geen Hallo verpakking overhead. |
| **Het BitsPerSample**<br/><br/> minInclusive = '0'<br/><br/> Vereist |**xs:int** |Bits per fragment voor Hallo wFormatTag indeling typt. |
...
| **IntegratedLoudnessSpeechPercentage**<br/><br/> minInclusive = "0" maxInclusive = "100" |**xs:float** |Spraak inhoud via Hallo programma, als een percentage. |
...
## <a name="xml"></a>XML-voorbeeld
 Hallo Hieronder volgt een voorbeeld van Hallo metagegevens uitvoerbestand.  
