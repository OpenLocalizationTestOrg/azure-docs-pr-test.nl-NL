...
In deze zelfstudie wordt gedemonstreerd hoe tooidentify client bepaalde fouten die mogelijk van invloed op prestaties en los deze fouten op end-to-end met de hulpprogramma's die worden geleverd door Microsoft en Azure Storage, in volgorde toooptimize Hallo-clienttoepassing.
...
* **Azure Storage Analytics**. [Azure Storage Analytics](http://msdn.microsoft.com/library/azure/hh343270.aspx) metrische gegevens en logboekregistratie voor Azure Storage biedt.

  * **Metrische gegevens Storage** houdt transactie metrische gegevens en capaciteitsmetrieken voor uw opslagaccount. Met metrische gegevens, kunt u bepalen hoe uw toepassing volgens tooa verscheidenheid aan andere maatregelen presteert. Zie [Storage Analytics metrische gegevens tabelschema](http://msdn.microsoft.com/library/azure/hh343264.aspx) voor meer informatie over metrische gegevens die worden bijgehouden door Storage Analytics Hallo typen.
  * **Opslag logboekregistratie** elk logboek met aanvragen toohello Azure Storage-services tooa serverzijde registreert. Hallo logboek houdt gedetailleerde gegevens voor elke aanvraag, waaronder Hallo bewerking uitgevoerd, Hallo status Hallo-werking en latentie informatie. Zie [Storage Analytics logboekindeling](http://msdn.microsoft.com/library/azure/hh343259.aspx) voor meer informatie over Hallo aanvraag- en gegevens die toohello Logboeken door Storage Analytics worden geschreven.
...
* **Klassieke Azure-Portal**. U kunt metrische gegevens en logboekregistratie configureren voor uw opslagaccount in Hallo [klassieke Azure-Portal](https://manage.windowsazure.com). U kunt ook grafieken en diagrammen die laten zien hoe uw toepassing wordt uitgevoerd na verloop van tijd weergeven en configureren van waarschuwingen toonotify die u als uw toepassing wordt uitgevoerd anders dan voor een opgegeven waarde verwacht.

    Zie [bewaken van een opslagaccount in Azure Portal Hallo](storage-monitor-storage-account.md) voor informatie over het configureren van de bewaking in Hallo klassieke Azure-Portal.
...
Voor deze zelfstudie kijken we naar de een scenario waarbij Azure Storage metrische gegevens staat voor een laag percentage slagingspercentage voor een toepassing die Azure storage aanroept. lage percentage geslaagd snelheid metriek Hallo (weergegeven als **PercentSuccess** in Hallo klassieke Azure-Portal en in Hallo metrische gegevens tabellen) houdt bewerkingen die slagen, maar die een HTTP-statuscode die groter is dan 299 retourneren. In logboekbestanden Hallo serverzijde opslag, deze bewerkingen worden geregistreerd met een transactiestatus **ClientOtherErrors**. Zie voor meer informatie over Hallo lage percentage geslaagd metriek [metrische gegevens tonen lage PercentSuccess of analytics logboekvermeldingen bewerkingen hebben met de status van ClientOtherErrors](storage-monitoring-diagnosing-troubleshooting.md#metrics-show-low-percent-success).
...
Let op Hallo lijsten hieronder staan verre voltooid. Zie [Status en foutcodes](http://msdn.microsoft.com/library/azure/dd179382.aspx) voor meer informatie over Azure Storage met algemene fouten en specifieke tooeach van opslagservices Hallo fouten.
...
Eerst moet u logboekregistratie van Azure Storage tooconfigure en metrische gegevens, zodat we gegevens uit Hallo client toepassing tooanalyze hebben. U kunt logboekregistratie en metrische gegevens configureren in tal van manieren - via Hallo [klassieke Azure-Portal](https://manage.windowsazure.com), met behulp van PowerShell, of via een programma. Zie [metrische gegevens Storage inschakelen en weergeven van metrische gegevens](http://msdn.microsoft.com/library/azure/dn782843.aspx) en [opslag vastleggen inschakelen en toegang tot logboekgegevens](http://msdn.microsoft.com/library/azure/dn782840.aspx) voor meer informatie over het configureren van logboekregistratie en metrische gegevens.

**Hallo via de klassieke Azure-Portal**

tooconfigure logboekregistratie en metrische gegevens voor uw opslagaccount Hallo Portal gebruikt, volgt u de instructies Hallo op [bewaken van een opslagaccount in Azure Portal Hallo](storage-monitor-storage-account.md).

> [!NOTE]
> Het is niet mogelijk tooset minuut metrieken met Hallo klassieke Azure-Portal. We raden echter aan dat u ze stelt voor Hallo doel van deze zelfstudie, en voor het onderzoeken van prestatieproblemen met uw toepassing. Minuut metrische gegevens met behulp van PowerShell, zoals hieronder, of programmatisch wilt instellen of Hallo via de klassieke Azure-Portal.
>
> Houd er rekening mee dat Hallo klassieke Azure-Portal minuut metrische gegevens en alleen per uur metrische gegevens kan niet worden weergegeven.
>
>
...
## <a name="review-metrics-data-in-hello-azure-classic-portal"></a>Metrische gegevens in de klassieke Azure-Portal Hallo controleren
Zodra uw toepassing actief is geweest gedurende een periode, kunt u lezen Hallo metrische gegevens grafieken die worden weergegeven in de klassieke Azure-Portal-tooobserve Hallo hoe uw service heeft is uitvoert. Eerst gaan we Hallo toevoegen **Verwerkingsfrequentie** metrische toohello bewaking pagina:

1. Navigeer toohello Dashboard voor uw opslagaccount in Hallo [klassieke Azure-Portal](https://manage.windowsazure.com), selecteer daarna **Monitor** tooview Hallo bewaking van de pagina.
2. Klik op **toevoegen metrische gegevens** toodisplay hello **metrische gegevens kiezen** dialoogvenster.
3. Schuif omlaag toofind hello **Verwerkingsfrequentie** groep, vouwen en selecteer **cumulatieve**, zoals wordt weergegeven in volgende Hallo-afbeelding. Met deze metriek cumuleert geslaagd percentagegegevens van alle Blob-bewerkingen.

![Kies de metrische gegevens](./media/storage-e2e-troubleshooting-classic-portal/choose-metrics-portal-1.png)

In de klassieke Azure-Portal hello, ziet u **Verwerkingsfrequentie** in Hallo grafiek bewaking, samen met andere metrische gegevens u hebt toegevoegd (up toosix kunnen worden weergegeven in de grafiek Hallo tegelijk). In Hallo volgende afbeelding ziet u dat Hallo percentage slagingspercentage is enigszins minder dan 100%, die we naast onderzoeken door te analyseren Hallo Logboeken in Message Analyzer Hallo-scenario:

![Diagram van de metrische gegevens in de portal](./media/storage-e2e-troubleshooting-classic-portal/portal-metrics-chart-1.png)

Zie voor meer informatie over het toevoegen van metrische gegevens toohello bewaking pagina [hoe: metrische gegevens toohello metrische gegevens tabel toevoegen](storage-monitor-storage-account.md#add-metrics-charts-to-the-portal-dashboard).

> [!NOTE]
> Het kan even duren voor de metrische gegevens tooappear in de klassieke Azure-Portal Hallo nadat u opslag metrische gegevens inschakelen. Dit komt omdat elk uur metrische gegevens voor Hallo vorige uur niet in Hallo klassieke Azure-Portal weergegeven worden totdat Hallo huidige uur is verstreken. Minuut metrische gegevens worden ook niet weergegeven in de klassieke Azure-Portal Hallo. Zodat het tootwo uren toosee metrische gegevens, afhankelijk van wanneer u metrische gegevens inschakelen duren kan.
>
>
...
![Message Analyzer sessie configureren](./media/storage-e2e-troubleshooting-classic-portal/configure-mma-session-1.png)
...
![Lay-out van Azure Storage weergeven](./media/storage-e2e-troubleshooting-classic-portal/view-layout-client-request-id-module.png)
...
![Lay-out van Azure Storage weergeven](./media/storage-e2e-troubleshooting-classic-portal/color-rules-menu.png)
...
![Lay-out van Azure Storage weergeven](./media/storage-e2e-troubleshooting-classic-portal/400-range-errors1.png)
...
![Lay-out van Azure Storage weergeven](./media/storage-e2e-troubleshooting-classic-portal/404-filtered-errors1.png)
...
![Gefilterde Server en de logboeken met traceringen](./media/storage-e2e-troubleshooting-classic-portal/server-filtered-404-error.png)
...
![Client logboek weergeeft 404-fouten](./media/storage-e2e-troubleshooting-classic-portal/client-log-analysis-grid1.png)
