...
Houd er rekening mee dat als Hallo map `C:\myfolder` niet bestaat, AzCopy wordt het maken en downloaden `abc.txt ` in Hallo nieuwe map.
...
Na de bewerking van de download hello, Hallo directory `C:\myfolder` Hallo volgende bestanden bevatten:
...
Hallo opgegeven bestemmingscontainer bestaat niet, wordt AzCopy deze maken als Hallo-bestand uploaden in de App.
...
Als Hallo virtuele map bestaat niet opgegeven, AzCopy hello tooinclude Hallo virtuele map in de naam wordt geüpload (*bijvoorbeeld*, `vd/abc.txt` in bovenstaande Hallo voorbeeld).
...
Optie `/S` uploads Hallo inhoud Hallo opgegeven directory tooBlob opslag recursief, wat betekent dat alle submappen en de bestanden ook wordt geüpload. Voor het exemplaar wordt ervan uitgegaan dat de volgende Hallo bestanden bevinden zich in map `C:\myfolder`:
...
Na het Hallo-uploadbewerking bevat Hallo container de Hallo volgende bestanden:
...
Als u geen optie opgeeft `/S`, AzCopy wordt recursief niet geüpload. Na het Hallo-uploadbewerking bevat Hallo container de Hallo volgende bestanden:
...
Als u geen optie opgeeft `/S`, AzCopy wordt alleen blobs die zich niet in een virtuele map bevinden uploaden:
...
Als u opgeeft `/SetContentType` zonder een waarde AzCopy wordt Stel elke blob of inhoudstype van het bestand volgens tooits bestandsextensie.
...
Na de kopieerbewerking hello bevat doelcontainer Hallo Hallo blob en het bijbehorende momentopnamen. Ervan uitgaande dat Hallo blob in bovenstaande Hallo voorbeeld heeft twee momentopnamen, omvatten Hallo container Hallo volgende blob en momentopnamen:
...
AzCopy standaard worden de gegevens tussen de twee eindpunten voor opslag asynchroon gekopieerd. Daarom Hallo kopieerbewerking wordt uitgevoerd op Hallo achtergrond met behulp van ongebruikte bandbreedtecapaciteit die geen SLA heeft in termen van hoe snel een blob worden gekopieerd en AzCopy controleert regelmatig de status van het exemplaar Hallo totdat Hallo kopiëren is voltooid of mislukt.

Hallo `/SyncCopy` optie zorgt ervoor dat de kopieerbewerking hello wordt consistente snelheid. AzCopy voert synchrone kopie Hallo Hallo blobs downloaden toocopy van Hallo opgegeven bron toolocal geheugen en uploadt deze toohello Blob-opslaglocatie.
...
Als Hallo bron is een Azure-bestandsshare opgegeven, moet u de exacte bestandsnaam hello, opgeven (*bijvoorbeeld* `abc.txt`) toodownload één bestand of de optie `/S` toodownload alle bestanden in de share Hallo recursief. Toospecify poging een patroon en de optie `/S` samen in een fout resulteert.
...
U kunt opgeven dat Hallo `/SyncCopy` optie toocopy gegevens van File Storage tooFile opslag van File Storage tooBlob opslag en van Blob Storage tooFile opslag synchroon, AzCopy doet dit door Hallo bron gegevens toolocal geheugen downloaden en te uploaden opnieuw toodestination. Standaard uitgaande kosten van toepassing.
...
Hallo partitioneren van index sleutel bereik is 0 als gebruiker niet de optie geeft `/PKRS`.
...
Houd er rekening mee dat Hallo mogelijke minimumwaarde voor de optie `/SplitSize` is 32 MB. Als Hallo opgegeven bestemming is Blob-opslag, AzCopy wordt gesplitst Hallo gegevensbestand één keer de grootte bereikt Hallo blob de maximale grootte (200GB), ongeacht of optie `/SplitSize` is opgegeven door de gebruiker Hallo.
...
Bij het opgeven van de indeling van nettolading CSV Hallo AzCopy genereren ook een schemabestand met extensie `.schema.csv` voor elk gegevensbestand.
...
AzCopy gelijktijdige bewerkingen tooexport entiteiten wordt gestart wanneer de gebruiker Hallo optie `/PKRS`. Elke bewerking exporteert een partitiesleutelbereik.
...
AzCopy wordt een JSON-gegevensbestand in blob-container met de volgende naamconventie Hallo gegenereerd:
...
Houd er rekening mee dat bij het exporteren van tabellen tooblobs AzCopy gedownload Hallo tabel entiteiten toolocal tijdelijke gegevensbestanden en vervolgens deze entiteiten toohello blob uploaden. Deze tijdelijke gegevensbestanden in Hallo journaal bestandsmap door het standaardpad Hallo zijn geplaatst '<code>%LocalAppData%\Microsoft\Azure\AzCopy</code>', kunt u optie/Z: [journaal-map] toochange Hallo journaal bestandsmaplocatie en dus wijzigen Hallo tijdelijke locatie van bestanden. Hallo tijdelijke gegevens voor de grootte van bestanden wordt bepaald door de grootte en het Hallo-grootte Hallo optie /SplitSize opgegeven, uw tabelentiteiten Hoewel Hallo tijdelijke gegevensbestand in de lokale schijf onmiddellijk worden verwijderd nadat deze is geüpload toohello blob, zorg dat u hebben voldoende ruimte lokale schijf toostore deze tijdelijke gegevensbestanden voordat ze worden verwijderd.
...
Als Hallo journal-bestand bestaat, controleert AzCopy of Hallo-opdrachtregel die ingevoerde overeenkomt met Hallo vanaf de opdrachtregel in Hallo journal-bestand. Als twee opdrachtregels Hallo overeenkomen, hervat AzCopy onvolledige Hallo-bewerking. Als ze niet overeenkomen, kunt u zich na vragen aan gebruiker tooeither overschrijven Hallo journaal bestand toostart een nieuwe bewerking of toocancel Hallo huidige bewerking.
...
Als Hallo bron is een Azure-bestandsshare opgegeven, moet u Hallo exacte bestandsnaam (bijvoorbeeld abc.txt) opgeven toocopy één bestand of het opgeven van de optie/s toocopy alle bestanden in Hallo share recursief. Toospecify poging een patroon en de optie leidt/s samen tot een fout opgetreden.
...
Als Hallo bron bron een blob-container is en een sleutel noch een SAS is opgegeven, wordt Hallo blob-container worden gelezen via anonieme toegang.
...
Hallo uitgebreide logboekbestand heet standaard AzCopyVerbose.log in `%LocalAppData%\Microsoft\Azure\AzCopy`. Als u een bestaande bestandslocatie voor deze optie opgeeft, worden uitgebreide Hallo toegevoegde toothat-bestand.  
...
Als deze optie niet opgegeven is of deze is opgegeven zonder het pad naar een map, maakt AzCopy Hallo journal-bestand op Hallo standaardlocatie bevindt, % LocalAppData%\Microsoft\Azure\AzCopy is.
...
### <a name="xn"></a>/XN
Sluit een nieuwere bronresource. Hallo resource wordt niet worden gekopieerd als hello laatst gewijzigd-tijd van de bron Hallo Hallo dezelfde of een nieuwer is dan het doel.

**Van toepassing op:** Blobs, bestanden

### <a name="xo"></a>/XO
Sluit een oudere bronserver resource. Hallo resource wordt niet worden gekopieerd als hello laatst gewijzigd-tijd van de bron Hallo Hallo dezelfde of een ouder zijn dan het doel.
...
Als deze optie niet is opgegeven, wordt de AzCopy tabel gegevens toosingle-bestand exporteren.

Als Hallo tabelgegevens geëxporteerde tooa blob is en hello geëxporteerde bestandsgrootte Hallo 200 GB limiet voor de Blobgrootte van de bereikt, wordt vervolgens AzCopy gesplitst Hallo geëxporteerde bestand, zelfs als deze optie is niet opgegeven.
...
Deze optie is optioneel tijdens de exportbewerking hello, AzCopy een manifestbestand met vooraf gedefinieerde naam wordt gegenereerd als deze optie niet is opgegeven.
...
Als u deze optie geen waarde opgeeft, wordt AzCopy ingesteld van elke blob of inhoudstype van het bestand volgens tooits bestandsextensie.
...
Voor de eigenschap 'AzureStorageUseV1MD5' • waar - maakt Hallo-standaardwaarde, AzCopy gebruik van MD5 .NET implementatie.
• ONWAAR – AzCopy gebruikt FIPS-compatibele MD5-algoritmen.
...
Raadpleeg voor meer informatie over Azure Storage en AzCopy toohello resources te volgen.

### <a name="azure-storage-documentation"></a>Documentatie bij Azure Storage:
* [Inleiding tooAzure opslag](storage-introduction.md)
* [Hoe toouse .NET Blob-opslag](storage-dotnet-how-to-use-blobs.md)
* [Hoe toouse File storage in .NET](storage-dotnet-how-to-use-files.md)
* [Hoe toouse Table storage uit het .NET](storage-dotnet-how-to-use-tables.md)
