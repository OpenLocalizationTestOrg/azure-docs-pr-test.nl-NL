...
Azure File storage biedt bestandsshares in de cloud met behulp van het standaard SMB-protocol. U kunt een bestandsshare koppelen van een client on-premises of in Azure. Zodra gekoppeld, kunnen hulpprogramma's zoals Robocopy worden gebruikt om bestanden te kopiëren via naar Azure-bestandsshares. Zie voor meer informatie [het koppelen van de Azure-bestandsshare op Windows](../files/storage-how-to-use-files-windows.md) en [het koppelen van Azure-bestandsshare op Linux](../storage-how-to-use-files-linux.md).
