...
U kunt tooyour apparaat via Windows PowerShell op afstand verbinding maken. Als u op deze manier verbinding maakt, ziet u niet een menu. (Ziet u een menu alleen als u de seriële console Hallo op Hallo apparaat tooconnect gebruiken.) Met Windows PowerShell op afstand, moet u specifieke runspace tooa verbinden. U kunt ook de weergavetaal Hallo opgeven.

Voor meer informatie over het gebruik van Windows PowerShell voor externe toegang toomanage uw apparaat gaat te[gebruik Windows PowerShell voor StorSimple tooadminister uw StorSimple-apparaat](storsimple-8000-windows-powershell-administration.md).

Deze zelfstudie wordt uitgelegd hoe tooconfigure uw apparaat voor extern beheer en klikt u vervolgens het tooconnect tooWindows PowerShell voor StorSimple. Kunt u HTTP of HTTPS tooremotely verbinden via Windows PowerShell. Echter, wanneer u beslist hoe tooconnect tooWindows PowerShell voor StorSimple, overweeg Hallo volgende informatie:
...
U kunt verbinden op afstand toohello Windows PowerShell-interface. Externe toegang tooyour StorSimple-apparaat via Windows PowerShell-interface Hallo is echter niet standaard ingeschakeld. U moet eerst extern beheer op Hallo apparaat inschakelen en klik vervolgens op Hallo client die wordt gebruikt tooaccess uw apparaat.
...
U kunt beide hello Azure portal of Hallo seriële console tooconfigure extern beheer gebruiken. Selecteer een van de Hallo procedures te volgen:

* [Hello Azure portal tooenable extern beheer via HTTP gebruiken](#use-the-azure-classic-portal-to-enable-remote-management-over-http)
...
### <a name="use-hello-azure-portal-tooenable-remote-management-over-http"></a>Hello Azure portal tooenable extern beheer via HTTP gebruiken

Voer Hallo stappen te volgen in hello Azure portal tooenable extern beheer via HTTP.

#### <a name="tooenable-remote-management-through-hello-azure-portal"></a>tooenable extern beheer via hello Azure-portal

1. Ga tooyour Apparaatbeheer StorSimple-service. Selecteer **apparaten** en selecteer en klikt u op de gewenste tooconfigure voor extern beheer van Hallo-apparaat. Ga te**apparaatinstellingen > beveiliging**.
2. In Hallo **beveiligingsinstellingen** blade, klikt u op **extern beheer**.
3. In Hallo **extern beheer** blade ingesteld **extern beheer inschakelen** te**Ja**.
...
3. U wordt geïnformeerd over Hallo beveiligingsproblemen van het gebruik van HTTP-tooconnect toohello apparaat. Bevestig door op te geven wanneer u wordt gevraagd, **Y**.
...
* [Hello Azure portal tooenable extern beheer via HTTPS gebruiken](#use-the-azure-classic-portal-to-enable-remote-management-over-https)
...
### <a name="use-hello-azure-portal-tooenable-remote-management-over-https"></a>Hello Azure portal tooenable extern beheer via HTTPS gebruiken

Voer Hallo stappen te volgen in hello Azure portal tooenable extern beheer via HTTPS.

#### <a name="tooenable-remote-management-over-https-from-hello-azure-portal"></a>tooenable remote management via HTTPS van hello Azure-portal
...
5. Klik op... en klik vervolgens op **certificaat voor extern beheer downloaden**. Geef een locatie toosave dit bestand. U moet dit certificaat op de client of hostcomputer computer Hallo die u tooconnect toohello apparaat gebruikt tooinstall.
...
Elk van de voorgaande procedures, Hallo wordt hieronder beschreven.

#### <a name="tooimport-hello-certificate-on-hello-remote-host"></a>tooimport hello certificaat op de externe host Hallo
1. Met de rechtermuisknop op Hallo cer-bestand en selecteer **installeren certificaat**. Hallo Wizard Certificaat importeren wordt gestart.
...
2. Hallo IP-adres toohello apparaatclient van vertrouwde hosts toevoegen door te typen:
    ...
    Waarbij <*device_ip*> Hallo IP-adres van uw apparaat is; bijvoorbeeld: 
     ...
     `Set-Item wsman:\localhost\Client\TrustedHosts 10.126.173.90 -Concatenate -Force`
3. een nieuwe referentie toocreate typt u:
...
6. U toowait moet een paar minuten en vervolgens kunt u zich verbonden tooyour apparaat via HTTPS via SSL. U ziet een bericht dat aangeeft dat u verbonden tooyour apparaat.
...
* Meer informatie over [uw StorSimple-apparaat met behulp van Windows PowerShell tooadminister](storsimple-8000-windows-powershell-administration.md).
