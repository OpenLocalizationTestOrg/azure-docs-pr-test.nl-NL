...
| TCP 9354 |out |WAN |Ja |De uitgaande poort wordt gebruikt door het StorSimple-apparaat om te communiceren met de StorSimple Manager-service. |
...
<sup>3</sup> de controller vaste IP-adressen op uw StorSimple-apparaat moet routeerbaar zijn en verbinding maken met het Internet rechtstreeks of via de geconfigureerde webproxy. De vaste IP-adressen worden gebruikt voor het onderhoud van de updates op het apparaat. Als de apparaatcontrollers kunnen geen verbinding met Internet via de vaste IP-adressen, kunt u zich niet uw StorSimple-apparaat bijwerken.
...
De routering metrische algoritmen zijn verschillend afhankelijk van de versie van de software op uw StorSimple-apparaat uitgevoerd.

**Versies vóór de Update 1**

Dit omvat softwareversies vóór de Update 1, zoals de GA, 0,1, 0,2 of 0,3 release. De volgorde op basis van routering metrische gegevens is als volgt:
...
**Versies van de Update 1 en vóór de Update 2 wordt gestart**

Dit omvat softwareversies zoals 1, 1.1 of 1.2. De volgorde op basis van routering metrische gegevens wordt als volgt bepaald:
   ...
   *DATA 0 > laatste geconfigureerd 10 GbE-netwerkinterface > andere 10 GbE-netwerkinterface > laatste geconfigureerd 1 GbE-netwerkinterface > andere 1 GbE-netwerkinterface*
   ...
   Update 1, de route-metric van DATA 0 wordt gedaan de laagste; Daarom worden alle cloud-verkeer wordt doorgestuurd via DATA 0. Noteer dit als er meer dan één ingeschakeld voor de cloud netwerkinterface op uw StorSimple-apparaat.

**Vanaf Update 2 releases**

Update 2 heeft verschillende verbeteringen van netwerkverkeer en de routering metrische gegevens is gewijzigd. Het gedrag kan als volgt worden verklaard.
...
* De volgorde waarin het cloud-verkeer wordt gerouteerd via de netwerkinterfaces is:
    ...
    *de getallen tussen haakjes aangeven waar de respectieve routering metrische gegevens.*
...
* Een waarschuwing wordt ook gegeven op uw StorSimple-apparaat wanneer er een VIP-fout. Ga voor meer informatie naar [Naslaggids waarschuwing](storsimple-manage-alerts.md).
...
* Het iSCSI- en cloud-verkeer te isoleren door een speciale netwerkinterfaces op uw apparaat voor toegang tot iSCSI- en cloud. Zie voor meer informatie hoe [netwerkinterfaces wijzigen](storsimple-modify-device-config.md#modify-network-interfaces) op uw StorSimple-apparaat.
...
* Beide PCMs niet gelijktijdig verwijderen. De PCM-module bevat de back-up batterij-module. Wanneer zowel de PCMs afsluiten zonder accu beveiliging wordt verwijderd en de apparaatstatus wordt niet opgeslagen. Voor meer informatie over de batterij, gaat u naar [onderhouden van de back-up batterij module](storsimple-battery-replacement.md#maintain-the-backup-battery-module).
...
* Tijdens de vervanging, controleren de **hardwarestatus** in de **onderhoud** pagina voor de stations in de SSD's en HDD's. De status van een groen vinkje geeft aan dat de schijven zijn in orde of OK, terwijl een rood uitroepteken wijst wijst op een mislukte SSD of harde schijf.
...
* Tijdens een EBOD controller module-vervanging voortdurend de status controleren van het onderdeel in de StorSimple Manager-service door het openen van **onderhoud** > **hardwarestatus**.
...
* [Informatie over het implementeren van uw StorSimple-oplossing](storsimple-deployment-walkthrough-u2.md).
