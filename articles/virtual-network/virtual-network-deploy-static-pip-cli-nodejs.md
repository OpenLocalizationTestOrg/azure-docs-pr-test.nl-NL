...
---
# <a name="create-a-vm-with-a-static-public-ip-address-using-hello-azure-cli-10"></a>Een virtuele machine maken met een statische openbare IP-adres met hello Azure CLI 1.0
...
U kunt deze taak uitvoeren met hello Azure CLI 1.0 (in dit artikel) of Hallo [Azure CLI 2.0](virtual-network-deploy-static-pip-arm-cli.md). 

## <a name = "create"></a>Stap 1: uw script starten
U kunt downloaden Hallo volledige bash-script waarmee [hier](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/IaaS-Story/03-Static-public-IP/virtual-network-deploy-static-pip-arm-cli.sh). Voer Hallo stappen toochange Hallo script toowork in uw omgeving te volgen:
...
4. Hallo-netwerkinterface (NIC) maken voor Hallo VM in Hallo subnet met Hallo openbare IP-adres hierboven wordt gemaakt. Kennisgeving Hallo eerste reeks opdrachten zijn gebruikte tooretrieve hello **Id** van Hallo subnet die eerder is gemaakt.
...
1. Uitvoeren vanuit een console bash Hallo script hierboven.

    ```azurecli
    sh myscript.sh
    ```

2. Hallo onderstaande uitvoer moet worden weergegeven na een paar minuten.
