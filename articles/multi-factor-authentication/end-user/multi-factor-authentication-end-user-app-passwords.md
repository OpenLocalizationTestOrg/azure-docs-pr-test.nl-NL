...
Bepaalde niet-browsertoepassingen, zoals Hallo Apple systeemeigen e-mailclient die gebruikmaakt van Exchange Active Sync, ondersteunen momenteel geen multi-factor authentication-server. Multi-factor authentication is ingeschakeld per gebruiker. Dit betekent dat als een gebruiker is ingeschakeld voor multi-factor authentication en ze niet-browsertoepassingen toouse proberen, ze zich niet kan toodo dus. Een app-wachtwoord kunt deze toooccur.

Zodra u een app-wachtwoord hebt, kunt u deze gebruiken in plaats van het oorspronkelijke wachtwoord met deze niet-browsertoepassingen. Dit is omdat wanneer u zich voor verificatie in twee stappen registreert, bent u Microsoft niet toolet iedereen Meld u aan met uw wachtwoord gevraagd als ze ook Hallo tweede verificatie niet kunnen uitvoeren. Hallo Apple systeemeigen e-mailclient op uw telefoon kan niet aanmelden als u omdat deze kan niet om verificatie in twee stappen vragen. Hallo oplossing voor dit is een veiliger app-wachtwoord die u niet gebruikt toocreate dagelijkse, maar alleen voor apps die verificatie in twee stappen niet ondersteunt. Hallo app-wachtwoord gebruiken zodat apps kunnen multi-factor authentication overslaan en doorgaan toowork.
...
Hallo volgen een aantal dingen tooremember over het toouse app-wachtwoorden.

* U kunt uw eigen app-wachtwoorden niet maken. In plaats daarvan worden ze automatisch gegenereerd. Omdat u alleen tooenter Hallo app-wachtwoord eenmaal per app hebt, is het veiliger toouse een complexere, automatisch gegenereerd wachtwoord in plaats van een makkelijk te onthouden.
* Er is momenteel een limiet van 40 wachtwoorden per gebruiker. Als u toocreate een probeert Hallo limiet is bereikt, wordt u na vragen aan gebruiker toodelete van uw bestaande appwachtwoorden zijn voordat u een nieuwe maakt.
* U moet één appwachtwoord per apparaat, niet per toepassing. U kunt bijvoorbeeld één app-wachtwoord voor uw laptop maken en dit app-wachtwoord gebruiken voor alle toepassingen op die laptop. Maak vervolgens een tweede app-wachtwoord toouse voor al uw apps op het bureaublad. 
