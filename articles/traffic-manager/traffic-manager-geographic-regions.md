---
title: "hiërarchie van aaaCountry/de regio die wordt gebruikt door Azure Traffic Manager geografische routeringstype | Microsoft Docs"
description: "In dit artikel worden gebruikt door Azure Traffic Manager geografische routeringstype land/regio-hiërarchie"
services: traffic-manager
documentationcenter: 
author: kumudd
manager: timlt
editor: 
ms.assetid: 
ms.service: traffic-manager
ms.devlang: na
ms.topic: article
ms.tgt_pltfrm: na
ms.workload: infrastructure-services
ms.date: 03/22/2017
ms.author: kumud
ms.openlocfilehash: d09fc896951fa1086822d69316468e7a98861591
ms.sourcegitcommit: 523283cc1b3c37c428e77850964dc1c33742c5f0
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 10/06/2017
---
# <a name="countryregion-hierarchy-used-by-azure-traffic-manager-for-geographic-traffic-routing-method"></a>Land/regio hiërarchie door Azure Traffic Manager gebruikt voor geografische verkeersrouteringsmethode

In dit artikel worden Hallo landen en regio's die worden gebruikt door Hallo **geografisch** methode in Azure Traffic Manager. U kunt deze gegevens ook programmatisch ophalen door het aanroepen van de [REST-API van Azure Traffic Manager](https://docs.microsoft.com/rest/api/trafficmanager/). 

- WORLD(World)

    - GEO-EU(Europe)

        - AD(Andorra)

        - AL(Albania)

        - AT(Austria)

        - AX (Ålandeilanden)

        - BA (Bosnië en Herzegovina)

        - BE(Belgium)

        - BG(Bulgaria)

        - BY(Belarus)

        - CH(Switzerland)

        - CY(Cyprus)

        - CZ (Tsjechië)

        - DE(Germany)

        - DK(Denmark)

        - EE(Estonia)

        - ES(Spain)

        - Fi(Finland)

        - VOOR (Faeroër)

        - FR(France)

        - GB (Verenigd Koninkrijk)

            - GB-eng(England)

            - GB NIR(Northern Ireland)

            - GB-sct(Scotland)

            - GB-WLS(Wales)

        - GG(Guernsey)

        - GI(Gibraltar)

        - GR(Greece)

        - HR(Croatia)

        - HU(Hungary)

        - IE(Ireland)

        - IM (eiland Man)

        - IS(Iceland)

        - IT(Italy)

        - Je(Jersey)

        - Li(Liechtenstein)

        - LT(Lithuania)

        - LU(Luxembourg)

        - LV(Latvia)

        - MC(Monaco)

        - MD(Moldova)

        - ME(Montenegro)

        - MK (Macedonië, FYRO)

        - MT(Malta)

        - NL(netherlands)

        - No(Norway)

        - PL(Poland)

        - PT(Portugal)

        - RO(ROMANIA)

        - RS(Serbia)

        - RU(Russia)

        - SE(Sweden)

        - SI(Slovenia)

        - SJ(Svalbard)

        - SK(Slovakia)

        - SM (San Marino)

        - UA(Ukraine)

        - VA (Vaticaanstad)

        - XJ (Jan Mayen)

        - XK(Kosovo)

    - GEO-ME(Middle East)

        - AE (United Arabische VAE)

        - BH(Bahrain)

        - IL(Israel)

        - IQ(Iraq)

        - IR(Iran)

        - Jo(Jordan)

        - KW(Kuwait)

        - LB(Lebanon)

        - OM(Oman)

        - PS (Palestijnse)

        - QA(Qatar)

        - Sy(Syria)

        - SA (Saudi-Arabië)

        - TR(Turkey)

        - Ye(Yemen)

    - GEO-NA(North America / Central America / Caribbean)

        - AG (Antigua en Barbuda)

        - AI(Anguilla)

        - AW(Aruba)

        - BB(Barbados)

        - BL (Saint-Barthélemy)

        - BM(Bermuda)

        - BQ(Bonaire)

        - BS(Bahamas)

        - BZ(Belize)

        - CA(Canada)

            - CA-AB(Alberta)

            - CA-BC(British Columbia)

            - CA-MB(Manitoba)

            - CA-NB(New Brunswick)

            - CA-NL(Newfoundland and Labrador)

            - CA-NS(Nova Scotia)

            - CA-NT(Northwest Territories)

            - CA-nu(Nunavut)

            - CA-on(Ontario)

            - CA-PE(Prince Edward Island)

            - CA-QC(Québec)

            - CA-SK(Saskatchewan)

            - CA-YT(Yukon Territory)

        - CR (Costa Rica)

        - Cu(Cuba)

        - Cw(CuraÃ§ao)

        - DM(Dominica)

        - DOEN (Dominicaanse Republiek)

        - GD(Grenada)

        - GL(Greenland)

        - GP(Guadeloupe)

        - GT(Guatemala)

        - HN(Honduras)

        - HT(Haiti)

        - JM(Jamaica)

        - KN (Saint Kitts en Nevis)

        - Tenantsleutel (Caymaneilanden)

        - Kredietbrief (Saint Lucia)

        - MF (Saint to)

        - MQ(Martinique)

        - MS(Montserrat)

        - MX(Mexico)

        - Ni(Nicaragua)

        - Pa(Panama)

        - CB (Saint Pierre en Miquelon)

        - PR (Portorico)

        - SV (El Salvador)

        - SX (Sint Maarten)

        - TC (Turks-en Caicoseilanden)

        - TT (Trinidad en Tobago)

        - UM (VERENIGDE STATEN Afgelegen eilanden)

        - US(United States)

            - US-Ak(Alaska)

            - US-al(Alabama)

            - US-ar(Arkansas)

            - US-AZ(Arizona)

            - US-CA(California)

            - US-CO(Colorado)

            - US-CT(Connecticut)

            - VS-DC(District of Columbia)

            - US-de(Delaware)

            - US-fl(Florida)

            - US-Ga(Georgia)

            - US-Hi(Hawaii)

            - US-IA(Iowa)

            - US-id(Idaho)

            - US-IL(Illinois)

            - US-in(Indiana)

            - US-KS(Kansas)

            - US-KY(Kentucky)

            - US-la(Louisiana)

            - US-ma(Massachusetts)

            - US-MD(Maryland)

            - US-ME(Maine)

            - US-MI(Michigan)

            - US-Mn(Minnesota)

            - US-mo(Missouri)

            - US-MS(Mississippi)

            - US-MT(Montana)

            - VS-NC(North Carolina)

            - VS-ND(North Dakota)

            - US-ne(Nebraska)

            - VS-NH(New Hampshire)

            - VS-NJ(New Jersey)

            - VS-NM(New Mexico)

            - US-NV(Nevada)

            - VS-NY(New York)

            - US-OH(Ohio)

            - US-OK(Oklahoma)

            - US-OR(oregon)

            - US-Pa(Pennsylvania)

            - VS-RI(Rhode Island)

            - VS-SC(South Carolina)

            - VS-SD(South Dakota)

            - US-tn(Tennessee)

            - US-TX(Texas)

            - US-UT(Utah)

            - US-VA(Virginia)

            - US-VT(Vermont)

            - US-WA(Washington)

            - US-Wi(Wisconsin)

            - VS-WV(West Virginia)

            - US-WY(Wyoming)

        - VC (Saint Vincent en de Grenadines Hallo)

        - VG (Brits Maagdeneilanden)

        - VI (VERENIGDE STATEN Eerste eilanden)

        - XE (Sint Eustatius)

        - XS(Saba)

    - GEO-as(Asia)

        - AF(Afghanistan)

        - AM(Armenia)

        - AZ(Azerbaijan)

        - BD(Bangladesh)

        - BN(Brunei)

        - BT(Bhutan)

        - CC (Cocoseilanden eilanden)

        - CN(China)

        - CX (Kerstmis eiland)

        - GE(Georgia)

        - HK (Hongkong SAR)

        - ID(Indonesia)

        - IN(India)

        - I/o (Brits Territorium in de Indische Oceaan)

        - JP(Japan)

        - KG(Kyrgyzstan)

        - KH(Cambodia)

        - KP (Noord Korea)

        - KR(Korea)

        - KZ(Kazakhstan)

        - La(Laos)

        - LK (Sri Lanka)

        - MM(Myanmar)

        - Mn(Mongolia)

        - MA (MACAU)

        - MV(Maldives)

        - My(Malaysia)

        - NP(Nepal)

        - PH(Philippines)

        - PK(Pakistan)

        - SG(Singapore)

        - TH(Thailand)

        - TJ(Tajikistan)

        - TL(Timor_Leste)

        - TM(Turkmenistan)

        - TW(Taiwan)

        - UZ(Uzbekistan)

        - VN(Vietnam)

    - GEO-af(Africa)

        - AO(Angola)

        - BF (Burkina Faso)

        - BI(Burundi)

        - BJ(Benin)

        - BV (Bouveteiland)

        - BW(Botswana)

        - CD (Congo-Kinshasa)

        - CF (Republiek Centraal Afrika)

        - CG(Congo)

        - CI (Ivoorkust)

        - CM(Cameroon)

        - K/l (Cabo Verde)

        - DJ(Djibouti)

        - Dz(Algeria)

        - EG(Egypt)

        - ER(Eritrea)

        - ET(Ethiopia)

        - Ga(Gabon)

        - GH(Ghana)

        - GM(Gambia)

        - GN(Guinea)

        - GQ (Equatoriaal-Guinea)

        - GW(Guinea_Bissau)

        - Ke(Kenya)

        - KM(Comoros)

        - LR(Liberia)

        - LS(Lesotho)

        - LY(Libya)

        - MA(Morocco)

        - MG(Madagascar)

        - ML(Mali)

        - MR(Mauritania)

        - MU(Mauritius)

        - MW(Malawi)

        - MZ(Mozambique)

        - NA(Namibia)

        - NE(Niger)

        - NG(Nigeria)

        - RE(Réunion)

        - RW(Rwanda)

        - SC(Seychelles)

        - SD(Sudan)

        - SH (Sint-Helena, Ascension en Tristan da Cunha)

        - SL (Sierraleoonse)

        - SN(Senegal)

        - SO(Somalia)

        - SS (Zuid Soedan)

        - ST (São Tomé en Príncipe)

        - SZ(Swaziland)

        - TD(Chad)

        - TF (Franse Zuidelijke en Zuidpoolgebieden)

        - TG(Togo)

        - TN(Tunisia)

        - TZ(Tanzania)

        - UG(Uganda)

        - YT(Mayotte)

        - ZA (Zuid-Afrika)

        - ZM(Zambia)

        - Zw(Zimbabwe)

    - GEO-an(Antarctica)

        - AQ(Antarctica)

    - GEO-SA(South America)

        - AR(Argentina)

        - BO(Bolivia)

        - BR(Brazil)

        - CL(Chile)

        - CO(Colombia)

        - EC(Ecuador)

        - FK (Falklandeilanden)

        - GF (Frans Guyana)

        - GS (Zuid-Georgië en de Zuidelijke Sandwich-eilanden)

        - GY(Guyana)

        - PE(Peru)

        - PY(Paraguay)

        - SR(Suriname)

        - UY(Uruguay)

        - VE(Venezuela)

    - GEO-AP(Australia / Pacific)

        - AS(American Samoa)

        - AU(Australia)

            - Australië-ACT(Australian Capital Territory)

            - Australië-NSW(New South Wales)

            - Australië-NT(Northern Territory)

            - AU-QLD(Queensland)

            - Australië-SA(South Australia)

            - AU-tas(Tasmania)

            - AU-VIC(Victoria)

            - Australië-WA(Western Australia)

        - St (Cookeilanden)

        - FJ(Fiji)

        - FM(Micronesia)

        - Gu(Guam)

        - HM (Heard- en McDonald-eilanden)

        - Ki(Kiribati)

        - DV (marshallen eilanden)

        - MP (Marianen)

        - NC (Nieuw-Caledonië)

        - NF (Norfolkeiland)

        - NR(Nauru)

        - NU(Niue)

        - NZ (Nieuw-Zeeland)

        - PF (Frans Polynesië)

        - PG (Papoea-Nieuw-Guinea)

        - PN (Pitcairneilanden)

        - PW(Palau)

        - SB (Salomonseilanden)

        - TK(Tokelau)

        - TO(Tonga)

        - TV(Tuvalu)

        - VU(Vanuatu)

        - WF (Wallis en Futuna)

        - WS(Samoa)

## <a name="next-steps"></a>Volgende stappen

- Meer informatie over [geografisch verkeer routeringsmethode in Azure Traffic Manager](traffic-manager-routing-methods.md#geographic).
