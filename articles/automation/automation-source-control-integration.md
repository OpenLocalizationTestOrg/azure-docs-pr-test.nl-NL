...
4. Nadat u de parameters hebt geconfigureerd, worden weergegeven op de **blade broncodebeheer instellen.**  
   
    ![Blade configureren](media/automation-source-control-integration/automation_02_SourceControlConfigure.png)
5. Wanneer u op OK klikt, worden de integratie van broncodebeheer is nu geconfigureerd voor uw Automation-account en moet worden bijgewerkt met uw GitHub-gegevens. U kunt nu op dit gedeelte om alle uw Taakgeschiedenis bron besturingselement synchronisatie weer te klikken.  
...
### <a name="check-in-a-runbook-from-azure-automation-to-source-control"></a>Een runbook uit Azure Automation voor broncodebeheer inchecken
Kunt u de wijzigingen die u hebt aangebracht in een runbook in Azure Automation in uw resourcebeheerbibliotheek push Runbook incheckt. Hieronder vindt u de stappen voor het selectievakje in een runbook:
...
2. Nadat u uw runbook hebt bewerkt, opslaan en klik op **inchecken** van de **bewerken** blade.  
   
    ![Knop inchecken](media/automation-source-control-integration/automation_06_CheckinButton.png)

     > [!NOTE] 
     > Inchecken van Azure Automation, wordt de code die momenteel aanwezig is in uw broncodebeheer overschreven. De instructie Git equivalente opdrachtregel in te checken is **git toevoegen + git-doorvoer + git push**  

1. Wanneer u klikt op **inchecken**, u wordt gevraagd met een bevestigingsbericht wordt weergegeven, klikt u op Ja om door te gaan.  
   
    ![Inchecken bericht](media/automation-source-control-integration/automation_07_CheckinMessage.png)
2. Controle begint het besturingselement bron runbook: **Sync MicrosoftAzureAutomationAccountToGitHubV1**. Dit runbook maakt verbinding met GitHub en wijzigingen van Azure Automation verstuurd naar uw opslagplaats. Als u wilt de controle-Taakgeschiedenis weergeven, gaat u terug naar de **integratie van broncodebeheer** tabblad en klik op om de blade opslagplaats synchronisatie. Deze blade bevat alle uw resourcebeheertaken.  Selecteer de taak die u wilt weergeven en klikt u op om de details weer te geven.  
   ...
   > 
   > 
3. De naam van het gewijzigde runbook wordt verzonden als invoerparameter aan het runbook incheckt. U kunt [de taakdetails weergeven](automation-runbook-execution.md#viewing-job-status-from-the-azure-portal) door het uitbreiden van runbook in **opslagplaats synchronisatie** blade.  
...
De knop synchronisatie op de blade opslagplaats synchronisatie kunt u voor het ophalen van alle runbooks in het pad naar de runbookmap van uw opslagplaats voor uw Automation-account. Dezelfde opslagplaats kan worden gesynchroniseerd met meer dan een Automation-account. Hieronder vindt u de stappen voor het synchroniseren van een runbook:

1. Open in het Automation-account waarmee u verbinding met broncodebeheer instellen, de **bron besturingselement integratie/opslagplaats synchronisatie blade** en klik op **Sync** en klik vervolgens wordt u gevraagd een bevestigingsbericht op **Ja** om door te gaan.  
   
    ![Knop synchronisatie](media/automation-source-control-integration/automation_10_SyncButtonwithMessage.png)
2. Het runbook door synchronisatie wordt gestart: **Sync MicrosoftAzureAutomationAccountFromGitHubV1**. Dit runbook maakt verbinding met GitHub en haalt de wijzigingen van uw opslagplaats voor Azure Automation. U ziet een nieuwe taak op de **opslagplaats synchronisatie** blade voor deze actie. Gedetailleerde informatie over de sync-taak, klik op om de blade taak details.  
   
    ![Runbook synchroniseren](media/automation-source-control-integration/automation_11_SyncRunbook.png)

    > [!NOTE] 
    > Een synchroniseren vanuit resourcebeheer overschrijft de conceptversie van de runbooks die momenteel beschikbaar zijn in uw Automation-account voor **alle** besturingselement voor runbooks die momenteel in de gegevensbron. De instructie Git equivalente opdrachtregel om te synchroniseren is **git, pull**


## <a name="troubleshooting-source-control-problems"></a>Het oplossen van problemen van bron-besturingselement
Als er fouten optreden met een in- of sync-taak, de status van de taak moet worden onderbroken en vindt u meer informatie over de fout in de taakblade.  De **alle logboeken** deel ziet u alle PowerShell-gegevensstromen die zijn gekoppeld met deze taak. Zodoende kunt u met de details om u te helpen u bij het oplossen van problemen met het in- of de synchronisatie. Deze ook ziet u de volgorde van de acties die zijn opgetreden tijdens het synchroniseren of controleren in een runbook.  
...
Als u wilt loskoppelen van uw GitHub-account, open de blade opslagplaats synchronisatie en klik op **Disconnect**. Wanneer u verbinding met broncodebeheer verbreken, runbooks die eerder zijn gesynchroniseerd blijven bestaan in uw Automation-account, maar de blade opslagplaats synchronisatie wordt niet ingeschakeld.  
