...
In deze zelfstudie gebruiken we [Python-Tools voor Visual Studio] toocreate een eenvoudige web-app met een van de PTVS-voorbeeldsjablonen Hallo worden opgevraagd. Deze zelfstudie is ook beschikbaar als een [video](https://www.youtube.com/watch?v=qUtZWtPwbTk).
...
2. Hallo projectsjablonen uit Hallo [Python-Tools 2.2 voor Visual Studio Samples VSIX] zijn beschikbaar onder **Python**, **voorbeelden**. Selecteer **Polls Flask-webproject** en klik op OK toocreate Hallo project.
...
2. Klik op Hallo **nieuw** pictogram op de voorgrond Hallo Hallo Portal links, klikt u op **gegevens en opslag** > **Opslagaccount**. Klik op **maken**, Hallo storage-account een unieke naam geven en maak een nieuwe [resourcegroep](../azure-resource-manager/resource-group-overview.md) voor.
...
3. Klik op Hallo **toegangstoetsen** deel in de blade Hallo van het opslagaccount. Let op het Hallo-accountnaam en het Hallo key1.
...
In deze sectie configureert we onze toepassing toouse hello opslagaccount die we zojuist hebben gemaakt. Bekijken we hoe tooobtain verbindingsinstellingen van hello Azure-Portal. Vervolgens voert we lokaal Hallo-toepassing.
...
1. In [Azure Portal](https://portal.azure.com), Hallo van web-app-blade geopend door te klikken op **Bladeren** > **App Services** > de naam van uw web-app.
...
Volg deze koppelingen toolearn meer over Python-Tools voor Visual Studio, Flask en Azure Table Storage.
