...
Controleer uw App Service om te bepalen of u van lokale Cache gebruikmaakt, **toepassingsinstellingen** tabblad. Als de lokale Cache wordt gebruikt, de app instellen `WEBSITE_LOCAL_CACHE_OPTION` is ingesteld op `Always`. Zie voor meer informatie over de lokale Cache, de [lokale App Service-Cache-overzicht](https://docs.microsoft.com/azure/app-service/app-service-local-cache).
