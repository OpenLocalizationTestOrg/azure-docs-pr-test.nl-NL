...
Meer informatie over de meestvoorkomende problemen Hallo en hun oplossingen bij het werken met Hadoop Distributed File System (HDFS) nettoladingen in Apache Ambari.

## <a name="how-do-i-access-local-hdfs-from-inside-a-cluster"></a>Hoe krijg ik toegang tot lokale HDFS uit in een cluster Hallo

### <a name="issue"></a>Probleem

Toegang Hallo lokale HDFS van Hallo opdracht regel- en code in plaats van met behulp van Azure Blob storage of Azure Data Lake Store uit binnen Hallo HDInsight-cluster.   

### <a name="resolution-steps"></a>Stappen voor het oplossen

1. Gebruik bij de opdrachtprompt Hallo `hdfs dfs -D "fs.default.name=hdfs://mycluster/" ...` letterlijk, zoals in de volgende opdracht Hallo:
...
2. Gebruik in de broncode, Hallo URI `hdfs://mycluster/` letterlijk, zoals in Hallo voorbeeldtoepassing te volgen:
...
3. Voer Hallo gecompileerde JAR-bestand (bijvoorbeeld een bestand met de naam `java-unit-tests-1.0.jar`) op Hallo HDInsight-cluster met Hallo volgende opdracht:
...
Hallo die lokale HDFS is vastgelopen in de veilige modus op Hallo HDInsight-cluster.   

### <a name="detailed-description"></a>Gedetailleerde beschrijving

Fout treedt op wanneer u Hallo na HDFS-opdracht uitvoeren:

```apache
hdfs dfs -D "fs.default.name=hdfs://mycluster/" -mkdir /temp
```

U ziet Hallo volgende fout wanneer u Hallo-opdracht uitvoert:
...
1. Hallo-status van HDFS op Hallo HDInsight-cluster met behulp van de volgende opdrachten Hallo ophalen:
...
2. Controleer Hallo integriteit van HDFS op Hallo HDInsight-cluster met behulp van Hallo volgende opdrachten:
