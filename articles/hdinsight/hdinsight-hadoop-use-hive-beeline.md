...
Beeline is een Hive-client die is opgenomen op de hoofdknooppunten van uw HDInsight-cluster. Beeline maakt JDBC verbinding met HiveServer2, een service die wordt gehost op uw HDInsight-cluster. U kunt ook Beeline gebruiken voor toegang tot de Hive in HDInsight op afstand via internet. De volgende tabel bevat de verbindingsreeksen voor gebruik met Beeline:

| Waar u Beeline van uitvoert | Parameters |
| --- | --- | --- |
| Een SSH-verbinding naar een headnode of edge-knooppunt | `-u 'jdbc:hive2://headnodehost:10001/;transportMode=http'` |
| Buiten het cluster | `-u 'jdbc:hive2://clustername.azurehdinsight.net:443/;ssl=true;transportMode=http;httpPath=/hive2' -n admin -p password` |
...
1. Bij het starten van Beeline, moet u een verbindingsreeks opgeven voor HiveServer2 op uw HDInsight-cluster. Voor het uitvoeren van de opdracht van buiten het cluster, moet u ook de naam van het cluster aanmelding account opgeven (standaard `admin`) en het wachtwoord. Gebruik de volgende tabel om te zoeken de indeling voor een verbindingsreeks en parameters moet worden gebruikt:
    ...
    De volgende opdracht kan bijvoorbeeld worden gebruikt voor Beeline van een SSH-sessie starten met het cluster:

    ```bash
    beeline -u 'jdbc:hive2://headnodehost:10001/;transportMode=http'
    ```

    Deze opdracht wordt de client Beeline gestart en verbinding maakt met HiveServer2 op het hoofdknooppunt van het cluster. Zodra de opdracht is voltooid, wordt u aankomt bij een `jdbc:hive2://headnodehost:10001/>` prompt.
...
4. Gebruik de volgende in het bestand met Beeline uit te voeren:
    ...
    > [!NOTE]
    > De `-i` parameter Beeline begint, voert u de instructies in het bestand query.hql. Nadat de query is voltooid, wordt u aankomt bij de `jdbc:hive2://headnodehost:10001/>` prompt. U kunt ook uitvoeren voor een bestand met de `-f` parameter Beeline afgesloten nadat de query is voltooid.
...
Als u deze Beeline lokaal zijn geïnstalleerd, of wordt gebruikt door een Docker-installatiekopie, zoals [sutoiku/beeline](https://hub.docker.com/r/sutoiku/beeline/), moet u de volgende parameters:
...
Spark biedt een eigen implementatie van HiveServer2, dit is vaak zoals als de Spark Thrift-server. Deze service wordt Spark SQL gebruikt voor query's in plaats van Hive omzetten en kan zorgen voor betere prestaties, afhankelijk van uw query.
