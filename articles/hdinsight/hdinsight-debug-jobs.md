...
Wanneer u een HDInsight-cluster maakt, worden automatisch 6 tabellen gemaakt voor op basis van Linux-clusters in de tabel standaard opslag:
...
* hadoopinstalllog: logboek van gebeurtenissen/uitzonderingen optreden tijdens het installeren van Hadoop op het cluster. Deze tabel mogelijk nuttig zijn bij het opsporen van problemen die betrekking hebben op clusters die zijn gemaakt met aangepaste parameters zijn.
* hadoopservicelog: logboek van gebeurtenissen/uitzonderingen worden bijgehouden door alle Hadoop-services. Deze tabel mogelijk nuttig zijn bij het opsporen van problemen met taakfouten op HDInsight-clusters.
...
**Power Query gebruiken om te openen en analyseren van het serviceaccount voor aanmelden**
...
3. Voer de naam van het opslagaccount. Dit kan zijn op de korte naam of de FQDN-naam.
...
5. Met de rechtermuisknop op de tabel hadoopservicelog in de **Navigator** deelvenster en selecteer **bewerken**. U ziet 4 kolommen. Verwijder desgewenst, de **partitiesleutel**, **rijsleutel**, en **tijdstempel** kolommen te selecteren en vervolgens op **kolommen verwijderen** uit de opties in het lint.
...
Klik op de naam van een HDInsight-cluster om de cluster-blade te openen vanuit de Azure-portal. Klik in de cluster-blade op **Dashboard**.

![Starten van de cluster-dashboard](./media/hdinsight-debug-jobs/hdi-debug-launch-dashboard.png)

Voer desgevraagd de referenties van de cluster-beheerder. In de Query-Console die wordt geopend, klikt u op **Hadoop UI**.
...
Klik op de naam van een HDInsight-cluster om de cluster-blade te openen vanuit de Azure-portal. Klik in de cluster-blade op **Dashboard**. Voer desgevraagd de referenties van de cluster-beheerder. In de Query-Console die wordt geopend, klikt u op **gebruikersinterface van YARN**.
...
Klik op de naam van een HDInsight HBase-cluster om de cluster-blade te openen vanuit de Azure-portal. Klik in de cluster-blade op **Dashboard**. Voer desgevraagd de referenties van de cluster-beheerder. In de Query-Console die wordt geopend, klikt u op **HBase UI**.

## <a name="hdinsight-error-codes"></a>HDInsight-foutcodes
De foutberichten die is gespecificeerd in deze sectie vindt u de gebruikers van Hadoop in Azure HDInsight mogelijke fouten die optreden kunnen bij het beheren van de service met Azure PowerShell te begrijpen en ze advies geven over de stappen die kunnen worden genomen van de fout wilt herstellen.
