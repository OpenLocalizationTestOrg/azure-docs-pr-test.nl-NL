...
Verifieer dat de gebruiker en apparaat goed hebt geconfigureerd en dat alle vereisten van het Enterprise State Roaming door het apparaat en de gebruiker wordt voldaan voordat u begint met het oplossen van problemen. 
...
2. Het apparaat is Azure AD zijn toegevoegd, of het domein en geregistreerd in Azure AD.
3. In de Azure Active Directory-portal **Enterprise State Roaming** is ingeschakeld voor de map onder **configureren** > **apparaten** > **gebruikers kunnen synchronisatie-instellingen en Enterprise-App-gegevens**. Alle gebruikers zijn geselecteerd, of de gebruiker is ingeschakeld voor het synchroniseren met behulp van de geselecteerde optie en opgenomen in de beveiligingsgroep.
4. De gebruiker heeft een Azure Active Directory Premium-abonnement aan hen toegewezen.  
5. Het apparaat is opnieuw opgestart en de gebruiker is aangemeld het nadat Enterprise State Roaming is ingeschakeld.

## <a name="information-to-include-when-you-need-help"></a>Informatie moet worden opgenomen wanneer u hulp nodig hebt
Als u niet op uw probleem met de onderstaande richtlijnen oplossen kunt, kunt u contact opnemen met onze ondersteuningsmedewerkers. Wanneer u contact opneemt met ze, wordt het aanbevolen om op te nemen van de volgende informatie:
...
- **Informatie over de gebruiker** – welke identiteit van de gebruiker voor aanmelding op het apparaat is? Hoe wordt de gebruiker zich bij het apparaat? Zijn ze deel uitmaken van een geselecteerde beveiligingsgroep kunnen synchroniseren? 
- **Informatie over het apparaat** : dit apparaat Azure AD join of het domein? Welke build is het apparaat op? Wat zijn de meest recente updates?
...
2. Controleer of de synchronisatie over meerdere machines werkt door een aantal wijzigingen op de oorspronkelijke computer, zoals het verplaatsen van de taakbalk op de bovenste of rechts van het scherm. Bekijk de wijziging wordt doorgegeven aan de tweede computer binnen vijf minuten. 
 ...
 - Moet u dezelfde aanmeldingsaccount op beide computers voor synchronisatie moet werken, zoals Enterprise State Roaming is gekoppeld aan het gebruikersaccount en niet de computeraccount.

**Potentiële probleem**: de instellingenpagina heeft de Schakelknoppen lichter gekleurd weergegeven en in plaats van een account te zien, ziet u de tekst 'enkele Windows-onderdelen zijn alleen beschikbaar als u een Microsoft-account of werkaccount'. Dit probleem kan ontstaan voor apparaten die worden ingesteld voor domein- en geregistreerde naar Azure AD, maar het apparaat niet is geverifieerd met Azure AD. Een mogelijke oorzaak is dat het apparaatbeleid moet worden toegepast, maar deze toepassing verloopt asynchroon en een paar uur vertraging kan optreden. Volg die de stappen in de status van het apparaat registratie om te controleren of deze controleren is om te controleren of dit probleem, het geval.
