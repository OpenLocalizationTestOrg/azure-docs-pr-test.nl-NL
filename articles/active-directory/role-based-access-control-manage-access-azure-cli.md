...
Voor het wijzigen van een aangepaste rol voor het eerst gebruiken de `azure role show` opdracht roldefinitie ophalen. Controleer vervolgens de gewenste wijzigingen aan in het definitiebestand voor de rol. Gebruik tot slot `azure role set` de gewijzigde roldefinitie opslaan.
...
U verwijdert een aangepaste rol door eerst gebruiken de `azure role show` opdracht om te bepalen de **ID** van de rol. Gebruik vervolgens de `azure role delete` opdracht voor het verwijderen van de rol door op te geven de **ID**.
