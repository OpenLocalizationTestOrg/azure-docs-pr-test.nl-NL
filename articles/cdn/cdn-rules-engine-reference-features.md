...
- Zorg ervoor dat de naam gedefinieerd in de optie waarde 
...
Uitgeschakeld|Zorgt ervoor dat op onze randservers voor het negeren van bandbreedte, snelheidsbeperking parameters. De aangevraagde inhoud wordt normaal gesproken worden geleverd (dat wil zeggen, zonder bandbreedtebeperking).
...
Hieronder vindt u Voorbeeldgebruik voor deze functie. Een voorbeeld van een aanvraag en de standaard Cachesleutel worden hieronder.
...
- Voordat u deze functie op een regel toevoegt, zorg voor de compressie uitgeschakeld opties instellen op de pagina compressie voor het platform waarop deze regel moet worden toegepast.
...
- Deze functie door configureren:
    ...
    - Een geheel getal opgeven en vervolgens te klikken op de gewenste tijdseenheid (dat wil zeggen, seconden, minuten, uren, enzovoort). Deze waarde bepaalt het standaardinterval voor de interne maximumleeftijd.
...
- Deze functie configureren door een geheel getal opgeven en het selecteren van de gewenste tijdseenheid (dat wil zeggen, seconden, minuten, uren, enzovoort).
...
- Deze functie door configureren:
    ...
    - Een geheel getal opgeven en het selecteren van de gewenste tijdseenheid (dat wil zeggen, seconden, minuten, uren, enzovoort). Deze waarde bepaalt van de aanvraag maximumleeftijd interval.
...
- Deze functie door configureren:
    ...
    - Een geheel getal opgeven en vervolgens te klikken op de gewenste tijdseenheid (dat wil zeggen, seconden, minuten, uren, enzovoort). Deze waarde bepaalt de interne max-verouderde die worden toegepast.
...
Waarde|Resultaat
--|--
Ingeschakeld|Zorgt ervoor dat de edge-server voor het ophalen van de asset opnieuw op de bronserver.
...
- Het gedrag van deze functie is afhankelijk van de geselecteerde tijdseenheid.
    - **Tijdseenheid:** een tijdsduur opgeven en selecteer een tijdseenheid (bijv, seconden, minuten, uren, enz.) om toe te staan van verouderde leveren van inhoud. Dit type installatieprogramma kan de CDN naar de tijdsduur die dit kan worden bezorgd uitbreiden inhoud voordat u validatie volgens de volgende formule:**TTL** + **verlopen tijdens de validatie opnieuw tijd** 
...
- Zorg ervoor dat alleen alfanumerieke tekens gebruiken.
...
-|-|-
Toevoegen|De opgegeven waarde worden toend van de bestaande waarde voor de aanvraag-header toegevoegd.|**Headerwaarde (Client) aanvraag:**Value1 <br/> **Headerwaarde (http-regelengine) aanvraag:** Value2 <br/>**Nieuwe aanvraag-header-waarde:** Value1Value2
...
- Aanvraag is niet in aanmerking voor de identificatie van een koptekst genomen. Elk van de volgende variaties van de naam van de Cache-Control-header kan bijvoorbeeld worden gebruikt bij het identificeren ervan:
    ...
    - CACHE-CONTROL
    - cachE-Control
- Zorg ervoor dat alleen alfanumerieke tekens, streepjes of onderstrepingstekens gebruiken bij het opgeven van een header-naam.
...
Optie|Beschrijving|Voorbeeld
-|-|-
Toevoegen|De opgegeven waarde worden toend van de bestaande waarde voor de aanvraag-header toegevoegd.|**Antwoord headerwaarde (Client):**Value1 <br/> **Antwoord headerwaarde (http-regelengine):** Value2 <br/>**Nieuwe antwoord headerwaarde:** Value1Value2
...
- Zorg ervoor dat de waarde die is opgegeven in de optie voor een exacte overeenkomst voor de gewenste response-header is. 
- Aanvraag is niet in aanmerking voor de identificatie van een koptekst genomen. Elk van de volgende variaties van de naam van de Cache-Control-header kan bijvoorbeeld worden gebruikt bij het identificeren ervan:
    - cache-control
    - CACHE-CONTROL
    - cachE-Control
...
Zorg dat de opgegeven header-naam komt niet overeen met het volgende:
...
Bron & patroon| Deze instellingen geven aan een aanvraag-URI-patroon waarmee het type van aanvragen die kunnen worden omgeleid. Alleen aanvragen waarvan de URL van de volgende criteria voldoet wordt omgeleid: <br/> <br/> **Bron:** (of toegang tot inhoud genoemd) Selecteer een relatief pad zijn dat een bronserver identificeert. Dit is de sectie '/XXXX/' en naam van uw eindpunt. <br/> **Bron (patroon):** een patroon dat aanvragen worden aangeduid met het relatieve pad moet worden gedefinieerd. Dit patroon van een reguliere expressie moet een pad dat begint direct na de eerder geselecteerde toegang tot inhoud wijst (Zie hierboven) definiëren. <br/> -Zorg ervoor dat de aanvraag-URI-criteria (dat wil zeggen, bron & patroon) hierboven gedefinieerde niet conflicteert met eventuele overeenkomst voorwaarden gedefinieerd voor deze functie. <br/> -Zorg ervoor dat een patroon opgeven. Met een lege waarde als het patroon wordt alleen gezocht naar aanvragen naar de hoofdmap van de oorsprong van de geselecteerde server (bijvoorbeeld http://cdn.mydomain.com/).
