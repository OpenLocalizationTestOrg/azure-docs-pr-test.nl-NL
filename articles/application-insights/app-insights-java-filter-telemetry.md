...
Filters bieden een manier tooselect Hallo telemetrie die uw [Java-web-app verzendt tooApplication Insights](app-insights-java-get-started.md). Er zijn een aantal out-of-the-box-filters die u kunt gebruiken en u kunt ook uw eigen aangepaste filters schrijven.

Hallo out-of-the-box filters omvatten:

* Traceerniveau ernst
* Specifieke URL's, trefwoorden of reactiecodes
* Snel op netwerkaanvragen - aanvragen dat wil zeggen toowhich uw app gereageerd tooquickly
* Namen van de specifieke gebeurtenis

> [!NOTE]
> Filters leiden tot onjuiste Hallo metrische gegevens van uw app. U kunt bijvoorbeeld besluiten dat de snelle responstijden van een filter-toodiscard in trage reacties in bindingsvolgorde toodiagnose, wordt ingesteld. Maar u moet rekening houden dat Hallo gemiddelde reactietijd van gerapporteerd door de Application Insights vervolgens langzamer dan Hallo ware snelheid zijn en Hallo aantal aanvragen kleiner dan de werkelijke telling Hallo is.
...
[Inspecteer de volledige set Hallo van ingebouwde processors](https://github.com/Microsoft/ApplicationInsights-Java/tree/master/core/src/main/java/com/microsoft/applicationinsights/internal/processor).
...
* `DurationThresholdInMS`-Duur verwijst toohello tijd tooload Hallo pagina. Als deze optie is ingesteld, worden pagina's die sneller dan de opgegeven tijd wordt geladen niet gerapporteerd.
* `NotNeededNames`-Met door komma's gescheiden lijst met paginanamen.
* `NotNeededUrls`-Door komma's gescheiden lijst met URL-fragmenten. Bijvoorbeeld: `"home"` gefilterd op alle pagina's die u hebt "home" Hallo-URL.
...
Alle telemetrie met waarden in Hallo SyntheticSource eigenschap gefilterd. Het gaat hierbij om aanvragen van bots, spiders en beschikbaarheidstests.
 ...
 *  TRACE - geen filters. tooTrace niveau is gelijk aan
...
### <a name="2-invoke-your-filter-in-hello-configuration-file"></a>2. Het filter in het configuratiebestand Hallo aanroepen
...
* Controleer of u geldige parameterwaarden hebt opgegeven. Duur moet bijvoorbeeld gehele getallen zijn. Ongeldige waarden zal Hallo filter toobe genegeerd. Als u een aangepaste filter er een uitzondering gegenereerd vanuit een constructor of een set-methode, worden genegeerd.
