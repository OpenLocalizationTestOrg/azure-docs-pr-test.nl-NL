...
Na het aanmelden is ziet u enkele gegevens op het scherm met de abonnements-id waarmee u zich hebt aangemeld in. Dit is het abonnement waarin de bronnen voor deze zelfstudie wordt worden gemaakt, tenzij u naar een ander abonnement wijzigen. Als u meerdere SubscriptionIds hebt, voer de volgende cmdlet voor een lijst van alle uw SubscriptionIds:

    Get-AzureRmSubscription

Als u wilt een andere abonnements-id, moet u de volgende cmdlet uitvoeren met de gewenste abonnements-id.
...
De volgende variabelen gebruiken voor het definiëren van de afbeelding moet worden gebruikt voor de virtuele machine. In dit voorbeeld wordt de installatiekopie van het SQL Server 2016 Enterprise gebruikt.
...
En u ziet de SKU's die beschikbaar zijn voor een aanbieding met de opdracht Get-AzureRmVMImageSku. De volgende opdracht toont alle SKU's beschikbaar zijn voor de **SQL2014SP1 WS2012R2** bieden.
