...
---
# <a name="sap-netweaver-on-azure-windows-virtual-machines-vms--planning-and-implementation-guide"></a>SAP NetWeaver op Windows Azure virtuele Machines (VM's) – Planning en implementatiehandleiding
...
Hallo papier zelf, ligt de nadruk op twee belangrijke aspecten:

* het eerste deel Hallo bevat een beschrijving van twee ondersteunde implementaties voor SAP NetWeaver op basis van toepassingen in Azure. Het wordt ook beschreven algemene verwerking van Azure met SAP-implementaties in gedachten.
* Hallo secondegedeelte wordt geïmplementeerd Hallo twee verschillende scenario's beschreven in het eerste deel Hallo beschreven.
...
* [SAP NetWeaver op Azure Virtual Machines (VM's) – plannen en implementatiehandleiding (in dit document)][planning-guide]
* [SAP NetWeaver op Azure virtuele Machines (VM's) – Implementatiehandleiding][deployment-guide]
* [SAP NetWeaver op Azure virtuele Machines (VM's) – DBMS Implementatiehandleiding][dbms-guide]
...
1. Gemiddelde grootte productiesystemen
2. Ontwikkeling van systemen
3. Testen van systemen
4. Prototypesystemen
5. Learning / demonstratie systemen

In de volgorde toosuccessfully implementeert SAP-systemen in Azure IaaS of IaaS in het algemeen, is het belangrijk toounderstand Hallo belangrijke verschillen tussen de offerings Hallo van traditionele uitbesteders of hosters en IaaS-aanbod. Terwijl Hallo traditionele hoster of outsourcer infrastructuur (type netwerk, opslag en server) toohello werkbelasting wordt pas een klant wil toohost, wordt deze in plaats daarvan van Hallo klant verantwoordelijkheid toochoose Hallo rechts werkbelasting voor IaaS-implementaties.
...
De meeste van deze gegevens vindt u [hier][virtual-machines-sizes]

Houd er rekening mee dat de grenzen die worden vermeld in de bovenstaande koppeling voor Hallo Hallo bovengrenzen zijn. Dit betekent niet dat Hallo beperkt voor Hallo bronnen, zoals IOPS onder alle omstandigheden kan worden opgegeven. Hallo-uitzonderingen zijn echter Hallo CPU en geheugenbronnen kost van een gekozen VM-type. Voor Hallo VM die worden ondersteund door SAP, zijn Hallo CPU en geheugenbronnen kost gereserveerd en als zodanig beschikbaar zijn op elk punt in tijd voor consumptie in Hallo VM.

Microsoft Azure-platform Hallo zoals andere platforms IaaS is een multitenant-platform. Dit betekent dat de opslag, netwerk en andere resources worden gedeeld tussen de tenants. Intelligent bandbreedtebeperking en quota logica is gebruikte tooprevent een tenant Hallo prestaties van een andere tenant (ruis neighbor) op een manier ingrijpende beïnvloeden. Hoewel de logica in Azure pogingen tookeep afwijkingen in kleine, maximaal gedeelde platforms van bandbreedte is vaak toointroduce groter afwijkingen in de beschikbare resource/bandbreedte dan een groot aantal klanten zijn gebruikte tooin hun on-premises-implementaties. Als gevolg hiervan kunt u verschillende niveaus van bandbreedte in groet toonetworking of opslag-i/o (Hallo volume evenals latentie) van de minuut toominute ervaren. Hallo-kans dat een SAP-systeem op Azure groter afwijkingen dan in een on-premises systeem optreden kan moet toobe in aanmerking genomen.
...
* Hallo SAP systemen (en VM(s)) vertegenwoordigen een zelfstandige scenario in Azure waarvoor alleen openbare verbinding met internet voor eindgebruikers toegang en vereist niet dat een verbinding tooother virtuele machines in Azure.
...
* In geval van demo en training scenario's die in meerdere virtuele machines, een Active Directory worden gerealiseerd Hallo / OpenLDAP en/of de DNS-service is vereist voor elke set van virtuele machines.
...
Dit scenario is een scenario voor Cross-Premises met veel mogelijke implementaties. Het kan zijn beschreven zo eenvoudig als het uitvoeren van sommige onderdelen van Hallo SAP liggend lokale en andere onderdelen van Hallo SAP Liggend op Azure. Alle aspecten van Hallo feit die deel uitmaken van Hallo SAP-onderdelen worden uitgevoerd op Azure moet transparant voor eindgebruikers. Daarom Hallo SAP Transport correctie System (STM's), RFC communicatie, afdrukken, beveiliging, (zoals SSO), enzovoort naadloos werkt voor Hallo SAP-systemen is uitgevoerd op Azure. Maar Hallo Cross-Premises scenario wordt ook beschreven voor een scenario waarbij Hallo voltooid SAP Liggend wordt uitgevoerd in Azure met van de klant Hallo domein en DNS uitgebreid naar Azure.

> [!NOTE]
> Dit is implementatiescenario Hallo die wordt ondersteund voor het uitvoeren van productief SAP-systemen.
>
>
...
>
> * Verschillende lagen van SAP-toepassingen in verschillende implementatiemethoden uitgevoerd. Bijvoorbeeld Hallo DBMS laag lokale, maar Hallo SAP toepassingslaag uitgevoerd in de virtuele machines die worden geïmplementeerd als Azure virtuele machines of vice versa.
> * Sommige onderdelen van een laag SAP in Azure en een on-premises. Bijvoorbeeld Exemplaren van Hallo SAP toepassingslaag tussen on-premises en Azure Virtual machines splitsen.
> * Distributie van virtuele machines met SAP-exemplaren van een systeem over meerdere Azure-regio's wordt niet ondersteund.
>
> Hallo reden voor deze beperkingen is Hallo vereiste voor een zeer lage latentie hoge prestaties netwerk binnen een SAP-systeem, met name tussen toepassingsexemplaren Hallo en Hallo DBMS laag van een SAP-systeem.
>
>
...
Hello Azure-platform vermindert de behoefte aan Hallo voor vooraf technologie en infrastructuur koopt. Het vereenvoudigt onderhouden en toepassingen door op te geven op aanvraag berekenings- en toohost besturingssysteem, uw webtoepassing en verbonden toepassingen beheren en schalen. Beheer van infrastructuur is geautomatiseerde met een platform dat is ontworpen voor hoge beschikbaarheid en dynamische schaling toomatch gebruik behoeften Hallo optie een betalen naar gebruik prijsmodel.
...
Van een operationeel perspectief ervaringen hello die Azure virtuele Machine Service vergelijkbare biedt als virtuele machines die on-premises worden geïmplementeerd. Dit heeft echter Hallo aanzienlijke voordeel dat u hoeft niet tooprocure, beheren en Hallo infrastructuur beheren. Ontwikkelaars en beheerders hebben volledig beheer van de installatiekopie van besturingssysteem Hallo binnen deze virtuele machines. Beheerders kunnen op afstand aanmelden bij deze virtuele machines tooperform-onderhoud en probleemoplossing van taken, evenals de taken van software-implementatie. In inachtneming van toodeployment zijn alleen beperkingen Hallo Hallo grootten en mogelijkheden van Azure Virtual machines. Deze mogelijk niet als fijn gedetailleerde in configuratie, zoals dit on-premises kan worden uitgevoerd. Er is een keuze uit de VM-typen die een combinatie van vertegenwoordigen:
...
Hallo grootte en beperkingen van verschillende grootten voor verschillende virtuele machines die worden aangeboden kunnen worden weergegeven in een tabel in [in dit artikel][virtual-machines-sizes]

Als u realiseren zijn er verschillende families of reeks van virtuele machines. Vanaf december 2015, kunt u onderscheiden Hallo families van virtuele machines te volgen:
...
* D-reeks VM typen: beter dan A0 A7 uitvoeren. Niet alle Hallo VM typen zijn gecertificeerd met SAP.
* DS-serie VM typen: gebruik dezelfde hosts als D-reeks, maar kunnen tooconnect tooAzure Premium-opslag zijn (Zie hoofdstuk [Azure Premium-opslag] [ planning-guide-3.3.2] van dit document). Opnieuw niet alle VM-typen zijn gecertificeerd met SAP.
* G-serie VM typen: veel geheugen VM typen.
* Typen GS-serie VM: G-serie, zoals maar inclusief Hallo optie toouse Azure Premium-opslag (Zie hoofdstuk [Azure Premium-opslag] [ planning-guide-3.3.2] van dit document). Bij gebruik van virtuele machines GS-serie als databaseservers is verplicht toouse Premium-opslag voor DB gegevens en transactie-logboekbestanden
...
Houd er rekening mee dat niet alle andere VM-reeks kan worden aangeboden in elk criterium van hello Azure-regio's (voor Azure-gebieden Zie volgende hoofdstuk). Zich bewust te zijn dat niet alle virtuele machines of VM-serie zijn gecertificeerd voor SAP.
...
Meer informatie over het gebruik van ARM-sjablonen vindt u hier:

* [Implementeren en beheren van virtuele machines met behulp van Azure Resource Manager-sjablonen en hello Azure CLI][virtual-machines-linux-cli-deploy-templates]
...
Een andere interessante functie is Hallo mogelijkheid toocreate installatiekopieën van virtuele Machines, waarmee u tooprepare bepaalde opslagplaatsen van waaruit u kunt tooquickly zijn implementeren van virtuele machine-serverexemplaren die aan uw vereisten voldoen.

Meer informatie over het maken van installatiekopieën van virtuele Machines kunt u vinden [in dit artikel (Windows)] [ virtual-machines-windows-capture-image] of in [in dit artikel (Linux)][virtual-machines-linux-capture-image].
...
Upgradedomeinen vertegenwoordigen een logische eenheid waarmee toodetermine hoe een virtuele machine binnen een SAP-systeem, die uit een SAP-instanties die actief zijn in meerdere virtuele machines bestaat, wordt bijgewerkt. Wanneer een upgrade optreedt, doorloopt Microsoft Azure Hallo-proces voor het bijwerken van deze Upgradedomeinen één voor één. U kunt uw SAP-systeem gedeeltelijk uit de mogelijke downtime beveiligen door virtuele machines tijdens de implementatie spreiden over verschillende domeinen upgraden. In order tooforce Azure toodeploy Hallo VM's van een SAP-systeem verdeeld over verschillende domeinen upgraden, moet u een specifiek kenmerk tooset tijdens de implementatie van elke virtuele machine. Vergelijkbare tooFault domeinen, een Azure-schaaleenheid is onderverdeeld in meerdere domeinen upgraden. In de volgorde toodirect hello Azure-infrastructuur controller toodeploy een set van virtuele machines via verschillende domeinen upgraden moet u tooassign een toohello Beschikbaarheidsset is ingesteld op Azure VM's tijdens de implementatie. Zie voor meer informatie over Azure Beschikbaarheidssets hoofdstuk [Beschikbaarheidssets van Azure] [ planning-guide-3.2.3] hieronder.

#### <a name="18810088-f9be-4c97-958a-27996255c665"></a>Azure Beschikbaarheidssets
Azure virtuele Machines binnen een Azure-Beschikbaarheidsset worden gedistribueerd door hello Azure-Infrastructuurcontroller via verschillende domeinen met fouten en domeinen upgraden. Hallo-doel van Hallo verdeling over de verschillende domeinen met fouten en domeinen upgraden is tooprevent die alle virtuele machines van een SAP-systeem wordt afgesloten in geval van Hallo van onderhoud van infrastructuur of een storing in een domein met fouten. Virtuele machines zijn standaard niet deel uit van een Beschikbaarheidsset. Hallo deelname van een virtuele machine in een Beschikbaarheidsset is gedefinieerd tijdens de implementatie of hoger op door een herconfiguratie en een nieuwe implementatie van een virtuele machine.

toounderstand hello concept van Beschikbaarheidssets van Azure en Hallo manier Beschikbaarheidssets betrekking tooFault en -domeinen upgraden hebben, Lees [in dit artikel][virtual-machines-manage-availability]
...
Microsoft Azure Virtual Machines gebruikmaken van verschillende opslagtypen. Bij het implementeren van SAP op Azure-Services voor virtuele Machine is het belangrijk toounderstand Hallo verschillen tussen deze twee typen opslag:
...
Instructies hierboven zijn toohello VM typen die zijn gecertificeerd met SAP toepassen. Hallo VM-serie met uitstekende IOPS en doorvoerlimieten in aanmerking voor gebruik door sommige DBMS-functies. Zie Hallo [DBMS Deployment Guide] [ dbms-guide] voor meer informatie.
...
Het is mogelijk tooupload een bestaande VHD on-premises of lege mappen uit in Azure maken en koppelen van deze VMs toodeployed. Deze virtuele harde schijven wordt verwezen als Azure-schijven.
...
* Zone redundante opslag die verschillende datacenters binnen verdeeld van installatiekopieën van de drie Hallo Hallo hetzelfde Azure-gebied.
* Redundantie standaardniveau is geografische redundantie dat asynchroon repliceert Hallo inhoud in een andere 3 afbeeldingen van gegevens in een ander Azure-regio die wordt gehost in Hallo Hallo dezelfde geopolitieke regio.

Zie ook Hallo tabel boven in dit artikel in groet toohello redundantie van de verschillende opties op: <https://azure.microsoft.com/pricing/details/storage/>

Meer informatie in groet tooAzure die opslag u hier vindt:
...
Azure standaard BLOB-opslag is Hallo type opslag beschikbaar bij Azure IaaS werd uitgebracht. Er zijn IOPS quota per één VHD afgedwongen. Latentie heeft ondergaan heeft niet dezelfde klasse als SAN/NAS-apparaten doorgaans geïmplementeerd voor geavanceerde SAP-systemen lokale gehoste Hallo. Niettemin hello Azure Standard-opslag voldoende voor veel honderden bewezen SAP-systemen ondertussen geïmplementeerd in Azure.

Azure Standard-opslag is in rekening gebracht op basis van Hallo de feitelijke gegevens die zijn opgeslagen, Hallo volume van opslagtransacties en uitgaande gegevensoverdracht van redundantie-optie gekozen. Veel VHD's kunnen worden gemaakt op Hallo maximaal 1 TB groot, maar als deze leeg blijft er zijn geen kosten. Als u vult u een VHD met 100GB, wordt u betalen voor het opslaan van 100GB en niet voor Hallo nominaal grootte Hallo die VHD met gemaakt.
...
In tegenover tooStandard opslag waar mogelijkheden is niet veranderd afhankelijk van het Hallo-grootte of Hallo schijf (VHD), Premium-opslag heeft momenteel 3 andere schijf categorieën die worden weergegeven aan einde van dit artikel voordat Hallo Veelgestelde vragen over sectie Hallo: <https:/ /Azure.Microsoft.com/Pricing/details/Storage/>

U ziet dat schijf- en IOPS/VHD doorvoer/VHD afhankelijk Hallo grootte categorie van het Hallo-schijven

Kosten basis in geval van Hallo van Premium-opslag is geen Hallo actuele gegevensvolume opgeslagen in een dergelijke VHD's, maar Hallo grootte categorie van dergelijke een VHD, onafhankelijk van Hallo hoeveelheid Hallo-gegevens die zijn opgeslagen in Hallo VHD.

U kunt virtuele harde schijven maken op Premium-opslag die niet rechtstreeks wilt toewijzen in Hallo grootte categorieën weergegeven. Kan dit Hallo geval, vooral wanneer virtuele harde schijven van de Standard-opslag worden gekopieerd naar Premium-opslag. In dergelijke gevallen wordt een toewijzing toohello volgende grootste Premium-opslag-schijfoptie uitgevoerd.

Zorg ervoor dat alleen bepaalde VM-reeks van hello Azure Premium-opslag profiteren kunt. Vanaf december 2015 zijn deze Hallo DS - en GS-serie. Hallo DS-serie is in feite Hallo dezelfde D-reeks met Hallo uitzondering dat DS-serie Hallo mogelijkheid toomount Premium-opslag heeft op basis van virtuele machines ook tooVHDs die worden gehost op Azure Standard-opslag. Hetzelfde geldt voor G-serie vergeleken tooGS-serie.

Als u Hallo-onderdeel van de DS-serie Hallo VM's in zijn uitgecheckt [in dit artikel] [ virtual-machines-sizes] ook wordt ontdekt u dat er gegevens volume beperkingen tooPremium opslag VHD's op Hallo granulatie van Hallo VM-niveau zijn. Andere DS-serie- of GS-serie VM's hebben andere beperkingen in groet toohello aantal virtuele harde schijven die kan worden gekoppeld. Deze limieten zijn gedocumenteerd in Hallo artikel ook hierboven vermeld. Maar in wezen betekent dat als u bijvoorbeeld 32 x P30 schijven/VHD's tooa koppelen één DS14 VM u geen 32 x Hallo maximale doorvoer van een schijf P30 krijgen kunt. In plaats daarvan beperkt hello maximale doorvoer op VM-niveau, zoals beschreven in artikel Hallo doorvoer van gegevens.
...
Microsoft Azure biedt een infrastructuur van het netwerk waarmee Hallo toewijzing van alle scenario's die we willen toorealize met SAP-software. Hallo-mogelijkheden zijn:
...
Cross-Premises waarbij die we Hallo fact zijn afhankelijk Hallo lokale AD en OpenLDAP/DNS-uitgebreid via VPN of een particuliere verbinding tooAzure. Voor bepaalde scenario's zoals hier wordt beschreven, is mogelijk noodzakelijk toohave een geïnstalleerd in Azure AD/OpenLDAP-replica.
...
* [Een virtuele machine maken met meerdere NIC 's][virtual-networks-multiple-nics]
...
Punt-naar-site VPN moet elke client machine tooconnect met een eigen VPN in Azure. Punt-naar-site-connectiviteit is geen praktische voor Hallo SAP scenario's met die we kijken. Daarom geen verdere verwijzingen krijgt toopoint-naar-site VPN-verbinding.
...
We moeten toobe wissen over Hallo feit die Hallo-opslag en infrastructuur van het netwerk wordt gedeeld tussen VM's met verschillende services in hello Azure-infrastructuur. En net zoals in Hallo van klant-datacenters overprovisioning van een aantal Hallo infrastructuur bronnen vindt plaats tooa mate. Hallo Microsoft Azure-Platform maakt gebruik van schijfruimte, CPU, netwerk- en andere quota toolimit Hallo brongebruik en toopreserve consistent en deterministisch prestaties.  Hallo verschillende VM typen (A5, A6, enzovoort) hebben verschillende quota's voor het Hallo aantal schijven, CPU, RAM-geheugen en het netwerk.

> [!NOTE]
> CPU en geheugen resources van Hallo VM die worden ondersteund door SAP zijn vooraf toegewezen op Hallo hostknooppunten. Dit betekent dat zodra Hallo VM is geïmplementeerd, Hallo-resources op Hallo host beschikbaar zijn zal zoals gedefinieerd door Hallo VM-type.
>
>

Wanneer planning en vergroten/verkleinen SAP op Azure-oplossingen Hallo quota's voor de grootte van elke virtuele machine moeten worden overwogen.  Hallo VM quota worden beschreven [hier][virtual-machines-sizes].

Hallo quota's beschreven vertegenwoordigen Hallo theoretische maximale waarden.  Hallo-limiet van IOP's per VHD kan worden bereikt met kleine IOs (8kb), maar mogelijk niet kan worden bereikt met grote IOs (1Mb).  Hallo IOPS limiet wordt afgedwongen op Hallo granulatie van één virtuele harde schijven.
...
Voor geïmplementeerde SAP-systemen, moet u hebben doorlopen een sizing oefening Hallo SAP's vereisten van Hallo system moet vast te stellen.
...
**Stap 4**: zoals beschreven [hier][virtual-machines-sizes], een quotum IOP's per onafhankelijk van de VHD door Azure worden afgedwongen of u Standard-opslag- of Premium-opslag gebruiken. Afhankelijk van Hallo VM-type, varieert van Hallo aantal VHD's die kunnen worden gekoppeld. Als gevolg hiervan kunt u het maximum aantal IOPS dat kan worden bereikt met elk van de andere VM-typen Hallo berekenen. Afhankelijk van bestandsopmaak Hallo-database, u kunt stripe VHD's toobecome één volume in Hallo gastbesturingssysteem. Echter, als huidige IOPS volume van een geïmplementeerde SAP-systeem Hallo Hallo berekend limieten van het grootste VM-type Hallo van Azure en als er geen toocompensate kans meer geheugen overschrijdt, Hallo werkbelasting Hallo SAP-systeem kan worden beïnvloed ernstige problemen. In dergelijke gevallen kunt u een punt waar moet u niet Hallo systeem in Azure implementeren bereikt.

**Stap 5**: vooral in SAP systemen die zijn geïmplementeerd op lokale in laag 2-configuraties, zijn Hallo kans op Hallo system die mogelijk nodig hebt geconfigureerd in Azure in een configuratie met 3-Laagse toobe. In deze stap moet u toocheck of er is een onderdeel in Hallo SAP toepassingslaag die niet worden uitgebreid en die niet in Hallo CPU en geheugen resources Hallo andere virtuele machine van Azure typen aanbieding zou passen. Als er inderdaad een dergelijke component, kunnen niet Hallo SAP-systeem en de werkbelasting worden geïmplementeerd in Azure. Maar als u kunt scale-out Hallo SAP-toepassing onderdelen in meerdere virtuele Azure-machines, Hallo-systeem kunnen worden geïmplementeerd in Azure.
...
Hello Azure Portal is een van drie interfaces toomanage Azure VM-implementaties. Hallo basic beheertaken, zoals het implementeren van virtuele machines van installatiekopieën, kunnen worden gedaan via hello Azure-Portal. Hallo maken van Opslagaccounts, virtuele netwerken en andere Azure-onderdelen zijn bovendien ook taken hello Azure Portal uitstekend kan verwerken. Echter functionaliteit zoals het uploaden van VHD's van lokale tooAzure of kopiëren van een VHD in Azure worden taken die het hulpprogramma's van derden of via PowerShell of CLI beheer vereist.
...
Naast het opnieuw opstarten of afsluiten van een virtuele Machine kunt u ook koppelen, loskoppelen en gegevensschijven voor Hallo-exemplaar voor virtuele Machine, toocapture Hallo-exemplaar voor de voorbereiding van de installatiekopie maken en configureren Hallo grootte van de instantie van de virtuele Machine Hallo.

Hello Azure Portal biedt basisfunctionaliteit toodeploy en virtuele machines en vele andere Azure-services configureren. Maar niet alle beschikbare functionaliteit wordt gedekt door hello Azure-Portal. In Azure Portal hello is het niet mogelijk tooperform taken, zoals:
...
Implementatie van hello Azure extensie Monitoring voor SAP (Zie hoofdstuk [Azure Monitoring Solution voor SAP] [ planning-guide-9.1] in dit document) is alleen mogelijk via PowerShell of CLI. Daarom is verplicht toosetup en PowerShell of CLI bij het implementeren of beheren van een systeem SAP NetWeaver in Azure te configureren.  

Als Azure meer functionaliteit biedt, gaan nieuwe PS-cmdlets toobe toegevoegd die moet worden bijgewerkt van Hallo-cmdlets. Daarom is het zin toocheck hello Azure downloaden site ten minste eenmaal Hallo maand <https://azure.microsoft.com/downloads/> voor een nieuwe versie van het Hallo-cmdlets. de nieuwe versie Hallo worden net boven op de oudere versie Hallo geïnstalleerd.

Voor een algemene lijst van Azure die betrekking hebben PowerShell-opdrachten controleren hier: <https://msdn.microsoft.com/library/azure/dn708514.aspx>.
...
Lees ook hoofdstuk [Azure CLI voor virtuele Linux-machines] [ deployment-guide-4.5.2] in Hallo [Deployment Guide] [ planning-guide] op hoe toouse Azure CLI toodeploy hello Azure Controle-extensie voor SAP.
...
U van plan bent een specifiek systeem SAP van lokale tooAzure toomove. Dit kan worden gedaan door Hallo VHD waarin Hallo OS uploaden, Hallo SAP binaire bestanden en DBMS binaire bestanden plus Hallo VHD's met Hallo gegevens en logboekbestanden van Hallo DBMS tooAzure. Daarentegen te[scenario #2 hieronder][planning-guide-5.1.2], houdt u Hallo hostnaam, SAP-SID en SAP-gebruikersaccounts in Azure VM Hallo zoals ze zijn geconfigureerd in Hallo on-premises omgeving. Hallo installatiekopie generaliseren is daarom niet nodig. Zie hoofdstukken [voorbereiding voor het verplaatsen van een virtuele machine van de lokale tooAzure met een schijf niet gegeneraliseerd] [ planning-guide-5.2.1] van dit document voor de voorbereidende stappen op lokale en uploaden van niet-gegeneraliseerde virtuele machines of virtuele harde schijven tooAzure. Lees hoofdstuk [Scenario 3: een virtuele machine verplaatsen van on-premises met een VHD Azure niet gegeneraliseerd met SAP] [ deployment-guide-3.4] in Hallo [Deployment Guide] [ deployment-guide]voor gedetailleerde stappen voor het implementeren van dergelijke een installatiekopie in Azure.

#### <a name="e18f7839-c0e2-4385-b1e6-4538453a285c"></a>Implementatie van een virtuele machine met een specifieke klant-installatiekopie
...
* VHD's die zijn gekoppeld toohello VM en opnieuw in Azure toohello VM nodig toobe in een vaste VHD-indeling ook moeten worden gekoppeld. Hallo geldt dezelfde maximale grootte van de besturingssysteemschijf Hallo ook toodata schijven. VHD's kunnen hebben een maximale grootte van 1TB. Dynamische virtuele harde schijven worden geconverteerd toostatic VHD's tijdens het uploaden van Hallo VHD met commandlets PowerShell of CLI
...
- - -
#### <a name="57f32b1c-0cba-4e57-ab6e-c39fe22b6ec3"></a>Voorbereiding voor het implementeren van een virtuele machine met een specifieke klant-installatiekopie voor SAP
VHD-bestanden die een gegeneraliseerde OS bevatten, worden ook opgeslagen in containers in Azure Storage-Accounts. U kunt een nieuwe VM implementeren vanaf deze installatiekopie van een VHD door te verwijzen Hallo VHD als een bron-VHD in uw implementatie sjabloonbestanden zoals beschreven in hoofdstuk [Scenario 2: een virtuele machine met een aangepaste installatiekopie implementeren voor SAP] [ deployment-guide-3.3] Hallo [Implementatiehandleiding][deployment-guide].
...
> ![Linux][Logo_Linux] Linux
>
> Zorg ervoor dat mnt/resourcegroep niet wordt gebruikt en dat alle schijven zijn gekoppeld via de uuid. Voor Hallo OS schijf controleren Hallo bootloader vermelding weerspiegelt ook Hallo uuid gebaseerde koppelen.
>
>

- - -
* SAP-GUI (voor administratieve en doeleinden setup) vooraf in deze sjabloon kan worden geïnstalleerd.
* Andere software nodig toorun Hallo VM's is in de Cross-Premises scenario's kan worden geïnstalleerd, zolang deze software met Hallo werken kunt Wijzig de naam van Hallo VM.
...
> ![Windows][Logo_Windows] Windows
>
> de laatste stap Hallo is toolog in tooa VM met een administratoraccount. Open een opdrachtvenster Windows als 'administrator'. Ga too...\windows\system32\sysprep en sysprep.exe uitvoeren.
...
> Als u wilt dat tooperform Hallo procedure met een VM die al worden uitgevoerd in Azure, stappen Hallo beschreven in [in dit artikel][virtual-machines-windows-capture-image].
>
> ![Linux][Logo_Linux] Linux
>
> [Hoe toocapture een toouse Linux virtuele machine als Resource Manager-sjabloon][virtual-machines-linux-capture-image]
>
>
...
Een derde alternatieve zou worden toouse verschillende van derden georiënteerde GUI-hulpprogramma's. Echter, zorg dat deze hulpprogramma's zijn ondersteuning van Azure-pagina-Blobs. Voor het opslaan van onze toepassing moet toouse Azure-pagina-Blob (Hallo verschillen worden hier beschreven: <https://msdn.microsoft.com/library/windowsazure/ee691964.aspx>). Hallo-hulpprogramma's van Azure zijn ook zeer efficiënt voor het comprimeren van Hallo virtuele machines en VHD's die toobe geüpload moeten. Dit is belangrijk omdat deze efficiëntie in compressie Hallo-tijd voor uploaden vermindert (die toch varieert, afhankelijk van Hallo uploaden koppeling toohello internet vanaf Hallo lokale faciliteit en hello Azure-implementatie regio gericht). Het is een evenwichtige veronderstelling dat een virtuele machine of een VHD uploaden van Europese locatie toohello VS op basis van Azure data centers duurt langer dan het uploaden van dezelfde virtuele machines/VHD's toohello Europese Azure-datacenters Hallo.
...
* Aanmelding tooyour abonnement met *Login-AzureRmAccount*
* Hallo-abonnement van uw context met ingesteld *Set-AzureRmContext* en parameter abonnements-id of SubscriptionName - Zie <https://msdn.microsoft.com/library/mt619263.aspx>
* Uploaden Hallo VHD met *toevoegen AzureRmVhd* tooan Azure Storage-Account - Zie <https://msdn.microsoft.com/library/mt603554.aspx>
* Set Hallo OS-schijf van een nieuwe VM config toohello VHD met *Set AzureRmVMOSDisk* -Zie <https://msdn.microsoft.com/library/mt603746.aspx>
* Een nieuwe virtuele machine maken van VM-configuratie met Hallo *New-AzureRmVM* -Zie <https://msdn.microsoft.com/library/mt603754.aspx>
* Voeg een schijf gegevens tooa nieuwe virtuele machine met *toevoegen AzureRmVMDataDisk* -Zie <https://msdn.microsoft.com/library/mt603673.aspx>

**Azure-CLI**

* Switch tooAzure Resource Manager-modus met *azure config mode arm*
* Aanmelding tooyour abonnement met *azure-aanmelding*
* Selecteer uw abonnement met *azure-account instellen`<subscription name or id`>*
* Uploaden Hallo VHD met *azure-opslag-blob uploaden* -Zie [Using hello Azure CLI met Azure Storage][storage-azure-cli]
* Maak een nieuwe VM geven Hallo VHD geüpload als besturingssysteemschijf met *azure vm maken* en de parameter -d
* Voeg een schijf gegevens tooa nieuwe virtuele machine met *vm schijf koppelen nieuwe*

**Sjabloon**

* Hallo VHD met Powershell of Azure CLI uploaden
* Hallo VM implementeren met een JSON-sjabloon die verwijzen naar Hallo VHD, zoals wordt weergegeven in [in dit voorbeeld JSON-sjabloon](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/201-vm-from-specialized-vhd/azuredeploy.json).
...
* Gebruik *sysprep* op Windows of *waagent-deprovision* op Linux toogeneralize uw VM - Zie [hoe toocapture virtuele Windows-machine in Hallo Resource Manager deployment model] [ virtual-machines-windows-capture-image-prepare-the-vm-for-image-capture] of [hoe toocapture een toouse Linux virtuele machine als Resource Manager-sjabloon][virtual-machines-linux-capture-image-capture]
* Aanmelding tooyour abonnement met *Login-AzureRmAccount*
* Hallo-abonnement van uw context met ingesteld *Set-AzureRmContext* en parameter abonnements-id of SubscriptionName - Zie <https://msdn.microsoft.com/library/mt619263.aspx>
* Uploaden Hallo VHD met *toevoegen AzureRmVhd* tooan Azure Storage-Account - Zie <https://msdn.microsoft.com/library/mt603554.aspx>
* Set Hallo OS-schijf van een nieuwe VM config toohello VHD met *Set AzureRmVMOSDisk - SourceImageUri - CreateOption fromImage* -Zie <https://msdn.microsoft.com/library/mt603746.aspx>
* Een nieuwe virtuele machine maken van VM-configuratie met Hallo *New-AzureRmVM* -Zie <https://msdn.microsoft.com/library/mt603754.aspx>

**Azure-CLI**

* Gebruik *sysprep* op Windows of *waagent-deprovision* op Linux toogeneralize uw VM - Zie [hoe toocapture virtuele Windows-machine in Hallo Resource Manager deployment model] [ virtual-machines-windows-capture-image-prepare-the-vm-for-image-capture] of [hoe toocapture een toouse Linux virtuele machine als Resource Manager-sjabloon] [ virtual-machines-linux-capture-image-capture] voor Linux
* Switch tooAzure Resource Manager-modus met *azure config mode arm*
* Aanmelding tooyour abonnement met *azure-aanmelding*
* Selecteer uw abonnement met *azure-account instellen`<subscription name or id`>*
* Uploaden Hallo VHD met *azure-opslag-blob uploaden* -Zie [Using hello Azure CLI met Azure Storage][storage-azure-cli]
* Maak een nieuwe VM geven Hallo VHD geüpload als besturingssysteemschijf met *azure vm maken* en de parameter -Q

**Sjabloon**

* Gebruik *sysprep* op Windows of *waagent-deprovision* op Linux toogeneralize uw VM - Zie [hoe toocapture virtuele Windows-machine in Hallo Resource Manager deployment model] [ virtual-machines-windows-capture-image-prepare-the-vm-for-image-capture] of [hoe toocapture een toouse Linux virtuele machine als Resource Manager-sjabloon] [ virtual-machines-linux-capture-image-capture] voor Linux
* Hallo VHD met Powershell of Azure CLI uploaden
* Hallo VM implementeren met een JSON-sjabloon die verwijzen naar Hallo installatiekopie VHD, zoals wordt weergegeven in [in dit voorbeeld JSON-sjabloon](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/101-vm-from-user-image/azuredeploy.json).

#### <a name="downloading-vhds-tooon-premises"></a>VHD's tooon-premises downloaden
...
Hallo VHD's kan niet tijdens Hallo van Hallo downloaden actief zijn. Zelfs bij het downloaden van VHD's die gekoppeld tooVMs zijn moet Hallo VM toobe afsluiten. Als u wilt dat alleen inhoud waarvan dan gebruikte tooset van een nieuw systeem on-premises en als het is acceptabel dat tijdens Hallo van Hallo downloaden en installatie van nieuwe Hallo Hallo systeem dat systeem in Azure Hallo kan nog steeds operationeel toodownload Hallo-database , u kunt een lange uitvaltijd voorkomen door het uitvoeren van een gecomprimeerde databaseback-up in een VHD en alleen downloaden dat VHD in plaats van gedownload ook Hallo OS base VM.

#### <a name="powershell"></a>PowerShell
Als Hallo SAP-systeem wordt gestopt en Hallo VM wordt afgesloten, kunt u Hallo PowerShell-cmdlet opslaan AzureRmVhd op Hallo lokale back-doel toodownload Hallo VHD schijven toohello lokale wereld. Toodo, moet u Hallo-URL van de VHD die u in Hallo 'Opslagsectie vinden kunt' Hallo Hallo Azure-Portal (nodig toonavigate toohello Storage-Account en Hallo storage-container waarin hello VHD is gemaakt) en u moet tooknow waar hello VHD moet worden gekopieerd naar.
...
Voor meer details van de cmdlet Hallo opslaan AzureRmVhd Controleer hier <https://msdn.microsoft.com/library/mt622705.aspx>.

#### <a name="cli"></a>CLI
Als Hallo SAP-systeem wordt gestopt en Hallo VM wordt afgesloten, kunt u hello Azure CLI opdracht azure-opslag-blob downloaden op Hallo lokale back-doel toodownload Hallo VHD schijven toohello lokale wereld. Toodo, moet u Hallo naam en het Hallo-container van VHD die u in Hallo 'Opslagsectie vinden kunt' Hallo Hallo Azure-Portal (nodig toonavigate toohello Storage-Account en Hallo storage-container waarin hello VHD is gemaakt) en u moet tooknow waar Hallo VHD moet worden gekopieerd naar.
...
### <a name="transferring-vms-and-vhds-within-azure"></a>Overdracht van virtuele machines en VHD's in Azure
#### <a name="copying-sap-systems-within-azure"></a>SAP-systemen binnen Azure kopiëren
Een SAP-systeem of zelfs een speciale DBMS-server een SAP-toepassingslaag ondersteunende wordt waarschijnlijk bestaan uit meerdere VHD's die gegevens Hallo of beide Hallo OS met Hallo binaire bestanden bevatten en logbestanden van Hallo SAP-database. Azure-functionaliteit van het kopiëren van virtuele harde schijven Hallo noch hello Azure functionaliteit van het opslaan van VHD's toodisk heeft een mechanisme voor synchronisatie die momentopname meerdere VHD's synchroon. Daarom Hallo status Hallo gekopieerd of VHD's opgeslagen, zelfs als die zijn gekoppeld tegen Hallo dezelfde virtuele machine zou afwijken. Dit betekent dat in concrete geval hebben verschillende gegevens en logfile(s) opgenomen in Hallo verschillende VHD's, -database in Hallo end Hallo Hallo inconsistent zijn zou.

**Conclusie: In volgorde toocopy of sla de VHD's die deel uitmaken van een systeemconfiguratie SAP u nodig hebt toostop Hallo SAP-systeem en moet ook geïmplementeerd tooshut omlaag Hallo VM. Alleen vervolgens kunt kopiëren of downloaden Hallo set tooeither VHD's een kopie maken van Hallo SAP-systeem in Azure of on-premises.**

Gegevensschijven kunnen worden opgeslagen als VHD-bestanden in een Azure Storage-Account en rechtstreeks tooa virtuele machine te koppelen of te worden gebruikt als een afbeelding. In dit geval is Hallo VHD gekopieerde tooanother locatie voordat beeing toohello virtuele machine gekoppeld. volledige naam van VHD-bestand in Azure Hallo Hallo moet uniek zijn binnen Azure. Zoals al eerder vermeld, is Hallo type een driedelige naam die lijkt op:

    http(s)://<storage account name>.blob.core.windows.net/<container name>/<vhd name>

##### <a name="powershell"></a>PowerShell
U kunt Azure PowerShell-cmdlets toocopy een VHD gebruiken zoals wordt weergegeven in [in dit artikel][storage-powershell-guide-full-copy-vhd].

##### <a name="cli"></a>CLI
U kunt Azure CLI toocopy een VHD gebruiken zoals wordt weergegeven in [in dit artikel][storage-azure-cli-copy-blobs]
...
Er zijn ook professional editions van Azure Storage Explorers die u kunt hier vinden:
...
Deze taak kan niet worden uitgevoerd op Hallo Azure-Portal. U kunt ise Azure PowerShell-cmdlets, Azure CLI of een derde partij browser opslag. Hallo PowerShell-cmdlets of de CLI-opdrachten kunnen maken en beheren van blobs, waaronder Hallo mogelijkheid tooasynchronously kopie BLOB's tussen Opslagaccounts en regio's binnen hello Azure-abonnement.

##### <a name="powershell"></a>PowerShell
Ook is het mogelijk om VHD's kopiëren tussen abonnementen. Lees voor meer informatie [in dit artikel][storage-powershell-guide-full-copy-vhd].

Hallo-basisstroom Hallo PS cmdlet logica ziet er als volgt:

* Maken van een context storage-account voor Hallo bron storage-account met *nieuw AzureStorageContext* -Zie <https://msdn.microsoft.com/library/dn806380.aspx>
* Maken van een context storage-account voor Hallo doel storage-account met *nieuw AzureStorageContext* -Zie <https://msdn.microsoft.com/library/dn806380.aspx>
...
#### <a name="4efec401-91e0-40c0-8e64-f2dceadff646"></a>VM/VHD-structuur voor SAP-implementaties
In het ideale geval Hallo afhandeling van Hallo-structuur van een virtuele machine en Hallo gekoppeld VHD's moeten zijn zeer eenvoudig. Klanten ontwikkeld in on-premises installaties tal van manieren van structureren van een server-installatie.

* Een basis-VHD waarin Hallo besturingssysteem en alle Hallo binaire bestanden van Hallo DBMS en/of SAP. Sinds maart 2015 mag deze VHD up too1TB in grootte in plaats van eerdere beperkingen beperkt het too127GB.
* Een of meerdere VHD's bevat Hallo DBMS-logbestand van Hallo SAP-database en het Hallo-logboekbestand Hallo DBMS tijdelijke opslaggebied (als Hallo DBMS wordt ondersteund). Als het logboek Hallo-IOPS Databasevereisten hoog zijn, moet u toostripe meerdere VHD's in volgorde tooreach hello IOPS volume dat vereist is.
* Een getal van VHD's met één of twee databasebestanden van Hallo SAP-database en Hallo DBMS tijdelijke gegevensbestanden ook (als Hallo DBMS dit ondersteunt).
...
- - -
Hallo-nummer van VHD's voor Hallo DBMS-gegevensbestanden en Hallo-type van deze virtuele harde schijven worden gehost op Azure-opslag moet worden bepaald door Hallo IOPS vereisten en Hallo latentie is vereist. Exacte quota worden beschreven in [in dit artikel][virtual-machines-sizes]

Ervaring van SAP implementaties via Hallo afgelopen 2 jaar geleerd ons enkele opgedane die kunnen worden samengevat als:

* IOPS verkeer toodifferent gegevensbestanden is niet altijd Hallo dezelfde omdat bestaande klantsystemen hebben anders gegevens grootte kunnen bestanden die hun SAP-database (s) vertegenwoordigt. Als gevolg hiervan bleek deze toobe beter gebruik van een RAID-configuratie via meerdere VHD's tooplace Hallo gegevensbestanden die LUN 's oppervlaktevariaties buiten die. Er zijn situaties, met name Azure Standard-opslag waar de snelheid van een IOPS Hallo quotum van een enkele VHD tegen Hallo DBMS transactielogboek bereikt. In dergelijke scenario's Hallo gebruik van Premium Storage aanbevolen of u kunt ook de aggregatie van meerdere standaard opslag VHD's met een software-RAID.
...
Houd er rekening mee dat Hallo VHD die Hallo besturingssysteem bevat en als het is raadzaam, Hallo binaire bestanden van SAP en Hallo-database (base VM), is niet meer beperkt too127GB. Deze kan nu hebben up too1TB in grootte. Dit is voldoende ruimte tookeep alle benodigde bestand met inbegrip van bijvoorbeeld SAP batch taaklogboeken Hallo.
...
> ![Windows][Logo_Windows] Windows
>
> [Hoe tooattach een schijf in hello Azure-portal][virtual-machines-windows-attach-disk-portal]
>
> Als de schijven zijn gekoppeld, moet u toolog in in Hallo VM tooopen Hallo Windows Schijfbeheer. Als automount niet is ingeschakeld zoals aanbevolen in hoofdstuk [instelling automount voor gekoppelde schijven][planning-guide-5.5.3], hello zojuist gekoppelde volume moet toobe online gemaakt en geïnitialiseerd.
>
> ![Linux][Logo_Linux] Linux
>
> Als de schijven zijn gekoppeld, u toolog in in Hallo VM nodig hebt en Hallo schijven initialiseren, zoals beschreven in [in dit artikel][virtual-machines-linux-how-to-attach-disk-how-to-initialize-a-new-data-disk-in-linux]
>
>
...
>
> Schakel Hallo documentatie van Hallo opdrachtregel uitvoerbare diskpart.exe hier in volgorde tooset Hallo automount:
>
> * [Opdrachtregelopties voor DiskPart](https://technet.microsoft.com/library/cc766465.aspx)
> * [Automount](http://technet.microsoft.com/library/cc753703.aspx)
>
> opdrachtregelvenster Windows Hello moet worden geopend als administrator.
>
> Als de schijven zijn gekoppeld, moet u toolog in in Hallo VM tooopen Hallo Windows Schijfbeheer. Als automount niet is ingeschakeld zoals aanbevolen in hoofdstuk [instelling automount voor gekoppelde schijven][planning-guide-5.5.3], Hallo zojuist gekoppeld volume > moet toobe online gemaakt en geïnitialiseerd.
>
> ![Linux][Logo_Linux] Linux
>
...
> * In de laatste stap van de wizard Hallo Hallo moet u toogive Hallo regel een naam en sla Hallo regel door te drukken 'Voltooid'
>
...
zoals beschreven in: <https://help.sap.com/saphelp_nwpi71/helpdata/en/47/c56a6938fb2d65e10000000a42189c/content.htm>
...
##### <a name="powershell"></a>PowerShell
* Maak een nieuwe groep resoure voor elke training/demo-liggend
...
#### <a name="setup-network-for-communication-between-hello-different-vms"></a>Setup-netwerk voor de communicatie tussen verschillende virtuele machines Hallo
...
### <a name="scenario-of-a-sap-landscape"></a>Scenario met een liggend SAP
Hallo Cross-Premises scenario kan ongeveer worden beschreven zoals Hallo Graphics hieronder:

![Site-naar-Site-connectiviteit tussen on-premises en Azure activa][planning-guide-figure-2100]

Hallo bovenstaande scenario wordt een scenario beschreven waarin Hallo lokale AD/OpenLDAP en DNS tooAzure wordt uitgebreid. Zijde van de lokale hello, is een bepaalde IP-adresbereik gereserveerd per Azure-abonnement. Hallo IP-adresbereik worden tooan Azure Virtual Network op Hallo Azure aan clientzijde toegewezen.
...
> * Volg Hallo openbare Linux handleidingen voor [SUSE](https://www.suse.com/documentation/sles-12/book_sle_deployment/data/sec_y2_hw_print.html) of [Red Hat](https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/6/html/Deployment_Guide/sec-Printer_Configuration.html) over het tooadd een printer.
>
>
...
RFC-verkeer tussen de systemen die zich on-premises en in Azure moet toowork. toosetup verbinding aanroep transactie SM59 in een bronsysteem waarin u toodefine moet een RFC-verbinding naar het doelsysteem Hallo. Hallo-configuratie is vergelijkbaar toohello standaardinstellingen van een RFC-verbinding.
...
Sinds februari 2014 Hallo 'Azure VM-Agent' waarmee afhandeling van specifieke Azure VM-extensies binnen Hallo die VM is opgenomen in Windows-VM's standaard op het maken van de VM in hello Azure-Portal. VM-agent is in geval van een SUSE of Red Hat Linux Hallo al deel uit van de Azure Marketplace-installatiekopie. Als een zou een Linux-VM uit uploaden beschikt lokale tooAzure Hallo VM-agent toobe handmatig geïnstalleerd.
...
Hallo initiële portal URI is http (s):`<Portalserver`>: 5XX00/irj waar Hallo poort wordt gevormd door 50000 plus (Systemnumber × 100). Hallo standaard portal URI van SAP-systeem 00 is `<dns name`>.`<azure region` >.Cloudapp.azure.com:PublicPort/irj. Voor meer informatie, hebt u een overzicht een <http://help.sap.com/saphelp_nw70ehp1/helpdata/de/a2/f9d7fed2adc340ab462ae159d19509/frameset.htm>.
...
* **Hoge beschikbaarheid van Azure-infrastructuur**, bijvoorbeeld HA van berekening (VM's), netwerk, opslag, enz. en de voordelen voor de beschikbaarheid van de SAP verhogen.
...
Er is geen enkel VM SLA beschikbaar op Azure Virtual Machines nu. tooget een idee hoe Hallo beschikbaarheid van één VM uitzien zoals u kunt gewoon opbouwen Hallo product van andere beschikbare Azure serviceovereenkomsten Hallo: <https://azure.microsoft.com/support/legal/sla/>.
...
Omdat Azure Storage is 3 afbeeldingen van Hallo gegevens houdt standaard, zijn RAID 5- of RAID1 over meerdere Azure-schijven niet nodig.
...
Als u geen toouse functies zoals Windows Server Failover Clustering (WSFC besluit) of als een Linux-equivalent (hello laatstgenoemde een wordt nog niet ondersteund op Azure in combinatie met SAP-software), de Azure VM starten gebruikte tooprotect een SAP-systeem tegen gepland en niet-geplande uitvaltijd van hello Azure fysieke serverinfrastructuur en algemene onderliggende Azure-platform.
...
Een ander belangrijk infrastructuur element voor hoge beschikbaarheid is opslag. Bijvoorbeeld SLA voor Azure Storage is 99,9% beschikbaarheid. Als er een implementeert alle virtuele machines met de schijven in één Azure Storage-Account, potentiële Azure Storage is niet beschikbaar zijn, wordt er geen beschikbaar zijn voor alle virtuele machines die in die Azure Storage-Account worden geplaatst en ook alle SAP onderdelen die op deze virtuele machines worden uitgevoerd.  
...
Een voorbeeldarchitectuur voor een SAP NetWeaver-systeem dat gebruikmaakt van Azure-infrastructuur HA kan er als volgt uit:
...
Elk omdat exemplaar wordt geplaatst in een eigen Azure Storage-account: potentiële ontbreken van een Azure Storage-Account wordt niet beschikbaar zijn van slechts één virtuele machine met de SAP-AS SAP-exemplaar. Let echter is er een limiet van Azure Storage-Accounts binnen een Azure-abonnement. tooensure automatisch wordt gestart (A) SCS exemplaar nadat Hallo VM opnieuw opstarten, zorg ervoor dat tooset Autostart parameter in (A) SCS-exemplaar dat wordt beschreven in hoofdstuk profiel starten [met behulp van Autostart voor SAP-exemplaren][planning-guide-11.5].
...
tooachieve volledige SAP hoge beschikbaarheid van het systeem, moeten we tooprotect na alle kritieke SAP systeemonderdelen, bijvoorbeeld redundante SAP-toepassingsservers en unieke onderdelen (bijvoorbeeld fouttolerantie), zoals SAP (A) SCS-exemplaar en DBMS.
...
Implementatie van een paar SAP-toepassingsserver exemplaren in hun toegewezen virtuele machines en ervan uitgaande dat wij 5 domeinen upgraden, Hallo volgende afbeelding Hallo achter ontstaat. Hallo werkelijke maximum aantal fouten en update domeinen binnen een beschikbaarheidsset kan worden gewijzigd in toekomstige Hallo:
...
* Alle virtuele machines (A) SCS-exemplaar van een SAP-systeem uitgevoerd, zijn in een Beschikbaarheidsset. Binnen van deze VMs is Windows Server Failover Cluster (WSFC) tooprotect (A) SCS exemplaar configureren.
...
Hallo-architectuur voor SAP HA op Linux in Azure is in feite Hallo hetzelfde als die voor Windows als hierboven beschreven. Vanaf januari 2016 zijn er twee beperkingen echter:

* alleen SAP as-omgeving 16 wordt momenteel ondersteund op Linux op Azure zonder de functies van de replicatie van een as-omgeving.
* Er is geen SAP (A) SCS HA oplossing nog ondersteund op Linux op Azure
...
Afhankelijk van Hallo SAP-configuratie gekozen (laag 2 of 3-Laagse) er mogelijk een toobackup nodig. Hallo-inhoud van virtuele machine zelf Hallo plus toohave een back-up van database Hallo. Hallo DBMS met betrekking tot back-ups zijn verwachte toobe klaar met de database-methoden. Een gedetailleerde beschrijving voor de verschillende databases hello, vindt u in [DBMS handleiding][dbms-guide]. Op Hallo daarentegen, Hallo SAP-gegevens kan een back-up op een offline manier (inclusief ook Hallo database-inhoud) zoals beschreven in deze sectie of online zoals beschreven in de volgende sectie Hallo.

Hallo offline back-ups zou in feite afsluiten Hallo VM via Azure Portal Hallo vereisen en een kopie van Hallo basisschijf VM plus alle VHD's toohello VM gekoppeld. Dit zou een punt in tijd installatiekopie Hallo VM en de bijbehorende schijf behouden. Back-ups' toocopy Hallo' wordt aangeraden in een andere Azure-Opslagaccount. Daarom Hallo procedure beschreven in hoofdstuk [kopiëren van schijven tussen Azure Storage-Accounts] [ planning-guide-5.4.2] van dit document wilt toepassen.
Afgezien van Hallo afsluiten met behulp van hello Azure-Portal kunt ook dit doen via Powershell of CLI zoals hier wordt beschreven: <https://azure.microsoft.com/documentation/articles/virtual-machines-deploy-rmtemplates-powershell/>

Terugzetten van deze staat kan bestaan uit van het verwijderen van Hallo base VM, evenals de oorspronkelijke schijven Hallo Hallo basis-VM en gekoppelde VHD's, kopiëren back Hallo opgeslagen virtuele harde schijven toohello oorspronkelijke Storage-Account en vervolgens opnieuw te implementeren Hallo systeem.
In dit artikel ziet u een voorbeeld het tooscript dit proces Powershell: <http://www.westerndevs.com/azure-snapshots/>

Zorg ervoor dat tooinstall een nieuwe SAP-licentie omdat restoing een VM-back-zoals hierboven beschreven een nieuwe hardwaresleutel maakt.

### <a name="online-backup-of-an-sap-system"></a>Online back-up van een SAP-systeem
Back-up van Hallo DBMS wordt uitgevoerd met specifieke DBMS-methoden zoals beschreven in Hallo [DBMS handleiding][dbms-guide].
...
> Vanaf januari 2016 Azure VM Backup biedt geen ondersteuning voor virtuele machines die zijn geïmplementeerd met Azure Resourc Manager nog.
>
> ![Windows][Logo_Windows] Windows
>
> Theoretisch virtuele machines die uitvoeren databases kunnen een back-up op een consistente manier ook als Hallo DBMS systemen ondersteunt Hallo Windows VSS (Volume Shadow Copy Service <https://msdn.microsoft.com/library/windows/desktop/bb968832 (v=vs.85).aspx > ) als u bijvoorbeeld SQL Server biedt.
> Let echter die op basis van de virtuele machine van Azure back-ups punt in tijd worden hersteld van de databases zijn niet mogelijk. De aanbeveling is daarom tooperform back-ups van databases met DBMS-functionaliteit in plaats van te vertrouwen op Azure VM Backup
>
> tooget bekend bent met de back-up van Azure virtuele Machine start hier: <https://azure.microsoft.com/documentation/articles/backup-azure-vms/>.
>
> Andere mogelijkheden zijn toouse een combinatie van Microsoft Data Protection Manager geïnstalleerd in een virtuele machine van Azure en Azure back-up naar back-up/herstel databases. Meer informatie vindt u hier: <https://azure.microsoft.com/documentation/articles/backup-azure-dpm-introduction/>.  
>
> ![Linux][Logo_Linux] Linux
>
> Er is geen equivalent tooWindows VSS in Linux. Alleen bestandsconsistente back-ups zijn daarom mogelijk, maar geen toepassingsconsistente back-ups. Hallo SAP DBMS back-up moet worden gedaan met behulp van de DBMS-functionaliteit. Hallo bestandssysteem waaronder Hallo SAP gerelateerde gegevens kunnen worden opgeslagen bijvoorbeeld met tar zoals beschreven hier: <http://help.sap.com/saphelp_nw70ehp2/helpdata/en/d3/c0da3ccbb04d35b186041ba6ac301f/content.htm>
>
>
