...
Hallo mogelijkheid tooredirect verkeer op basis van een gedefinieerde configuratie biedt ondersteuning voor de toepassingsgateway. meer informatie over omleiding in algemene bezoek toolearn [Application Gateway omleiding overzicht](application-gateway-redirect-overview.md). Dit artikel vindt u voorbeelden van HTTP-omleiding voor tooHTTPS, op basis van pad omleidingen, omleidingen van meerdere sites, en tooexternal sites omgeleid.

## <a name="http-toohttps-redirect-on-an-existing-application-gateway"></a>HTTP-tooHTTPS omleiden op een bestaande application gateway

Hallo volgende voorbeeld wordt een nieuwe HTTP-listener op poort 80 tooredirect aanvragen toohello bestaande listener op poort 443.
...
Nadat de toepassingsgateway hello wordt bijgewerkt, test u Hallo nieuwe listener op poort 80.  Hallo volgende voorbeeld ziet u met behulp van `curl` Hallo omleiding testen.
...
Hallo voorbeeld te volgen maakt een nieuwe listener en een padregel gebaseerd op een bestaande toepassingsgateway waarmee alleen de voldoen aan specifieke url-pad Hallo verkeer worden omgeleid.
...
## <a name="redirect-tooan-external-site"></a>De externe site tooan omleiden

Hallo volgende voorbeeld wordt omgeleid verkeer dat afkomstig is bij Hallo application gateway tooan externe website (bing.com). Wanneer u Hallo `TargetUrl` switch Hallo `IncludePath` en `IncludeQuery` switches zijn niet toegestaan.
...
Ga naar [end tooend SSL configureren met application gateway met PowerShell](application-gateway-end-to-end-ssl-powershell.md)
