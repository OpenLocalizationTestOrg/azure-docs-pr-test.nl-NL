---
title: aaaConfigure omleiding voor Azure Application Gateway - PowerShell | Microsoft Docs
description: Deze pagina vindt u scenario's tooconfigure omleiding voor Application Gateway met behulp van PowerShell
...
---
# <a name="configure-redirection-on-application-gateway-with-powershell"></a><span data-ttu-id="ac332-103">Omleiding configureren in Application Gateway met PowerShell</span><span class="sxs-lookup"><span data-stu-id="ac332-103">Configure redirection on Application Gateway with PowerShell</span></span>

<span data-ttu-id="ac332-104">Hallo mogelijkheid tooredirect verkeer op basis van een gedefinieerde configuratie biedt ondersteuning voor de toepassingsgateway.</span><span class="sxs-lookup"><span data-stu-id="ac332-104">Application gateway supports hello ability tooredirect traffic based on a defined configuration.</span></span> <span data-ttu-id="ac332-105">meer informatie over omleiding in algemene bezoek toolearn [Application Gateway omleiding overzicht](application-gateway-redirect-overview.md).</span><span class="sxs-lookup"><span data-stu-id="ac332-105">toolearn more about redirection in general visit, [Application Gateway redirect overview](application-gateway-redirect-overview.md).</span></span> <span data-ttu-id="ac332-106">Dit artikel vindt u voorbeelden van HTTP-omleiding voor tooHTTPS, op basis van pad omleidingen, omleidingen van meerdere sites, en tooexternal sites omgeleid.</span><span class="sxs-lookup"><span data-stu-id="ac332-106">This article provides examples of HTTP tooHTTPS redirection, path based redirects, multi-site redirects, and redirects tooexternal sites.</span></span>

## <a name="http-toohttps-redirect-on-an-existing-application-gateway"></a><span data-ttu-id="ac332-107">HTTP-tooHTTPS omleiden op een bestaande application gateway</span><span class="sxs-lookup"><span data-stu-id="ac332-107">HTTP tooHTTPS redirect on an existing application gateway</span></span>

<span data-ttu-id="ac332-108">Hallo volgende voorbeeld wordt een nieuwe HTTP-listener op poort 80 tooredirect aanvragen toohello bestaande listener op poort 443.</span><span class="sxs-lookup"><span data-stu-id="ac332-108">hello following example creates a new HTTP listener on port 80 tooredirect requests toohello existing listener on port 443.</span></span>
...
<span data-ttu-id="ac332-109">Nadat de toepassingsgateway hello wordt bijgewerkt, test u Hallo nieuwe listener op poort 80.</span><span class="sxs-lookup"><span data-stu-id="ac332-109">Once hello application gateway is updated, test hello new listener on port 80.</span></span>  <span data-ttu-id="ac332-110">Hallo volgende voorbeeld ziet u met behulp van `curl` Hallo omleiding testen.</span><span class="sxs-lookup"><span data-stu-id="ac332-110">hello following example shows using `curl` test hello redirection.</span></span>
...
## <a name="path-based-redirect"></a><span data-ttu-id="ac332-111">Padgebaseerde omleiding</span><span class="sxs-lookup"><span data-stu-id="ac332-111">Path based redirect</span></span>

<span data-ttu-id="ac332-112">Hallo voorbeeld te volgen maakt een nieuwe listener en een padregel gebaseerd op een bestaande toepassingsgateway waarmee alleen de voldoen aan specifieke url-pad Hallo verkeer worden omgeleid.</span><span class="sxs-lookup"><span data-stu-id="ac332-112">hello following example creates a new listener and a path based rule on an existing application gateway that redirects only traffic meeting hello specific url path.</span></span>
...
## <a name="multi-site-redirect"></a><span data-ttu-id="ac332-113">Multi-site-omleiding</span><span class="sxs-lookup"><span data-stu-id="ac332-113">Multi-site redirect</span></span>

<span data-ttu-id="ac332-114">Hallo volgende voorbeeld wordt een nieuwe toepassingsgateway met 2 multi-site-listeners op poort 80.</span><span class="sxs-lookup"><span data-stu-id="ac332-114">hello following example creates a new application gateway with 2 multi-site listeners on port 80.</span></span> <span data-ttu-id="ac332-115">Hallo-listeners zijn voor adatum.com en adatum.org. Een regel omleiding gemaakt tooredirect verkeer vanuit adatum.org tooadatum.com. Aanvullende configuratie is vereist voor het configureren van CNAME-aliassen toohello application gateway openbare IP-adres, voor meer informatie over het overdragen van uw domein tooAzure DNS- en maken van CNAME-records voor uw domein bezoekt, [een domein delegeren tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).</span><span class="sxs-lookup"><span data-stu-id="ac332-115">hello listeners are for adatum.com and adatum.org. A redirect rule is created tooredirect traffic from adatum.org tooadatum.com. Additional configuration is required for configuring CNAME aliases toohello application gateway public IP address, for more information on delegating your domain tooAzure DNS and creating CNAME records for your domain visit, [Delegate a domain tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).</span></span>
...
## <a name="redirect-tooan-external-site"></a><span data-ttu-id="ac332-116">De externe site tooan omleiden</span><span class="sxs-lookup"><span data-stu-id="ac332-116">Redirect tooan external site</span></span>

<span data-ttu-id="ac332-117">Hallo volgende voorbeeld wordt omgeleid verkeer dat afkomstig is bij Hallo application gateway tooan externe website (bing.com).</span><span class="sxs-lookup"><span data-stu-id="ac332-117">hello following example redirects traffic coming into hello application gateway tooan external website (bing.com).</span></span> <span data-ttu-id="ac332-118">Wanneer u Hallo `TargetUrl` switch Hallo `IncludePath` en `IncludeQuery` switches zijn niet toegestaan.</span><span class="sxs-lookup"><span data-stu-id="ac332-118">When using hello `TargetUrl` switch hello `IncludePath` and `IncludeQuery` switches are not allowed.</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="ac332-119">Volgende stappen</span><span class="sxs-lookup"><span data-stu-id="ac332-119">Next steps</span></span>

<span data-ttu-id="ac332-120">Ga naar [end tooend SSL configureren met application gateway met PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span><span class="sxs-lookup"><span data-stu-id="ac332-120">Visit [configure end tooend SSL with application gateway using PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span></span>
