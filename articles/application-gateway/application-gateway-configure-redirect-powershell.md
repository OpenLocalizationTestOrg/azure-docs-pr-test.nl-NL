---
title: Een omleiding voor Azure Application Gateway configureren - PowerShell | Microsoft Docs
...
---
# <a name="configure-redirection-on-application-gateway-with-powershell"></a><span data-ttu-id="005ef-103">Omleiding configureren in Application Gateway met PowerShell</span><span class="sxs-lookup"><span data-stu-id="005ef-103">Configure redirection on Application Gateway with PowerShell</span></span>

<span data-ttu-id="005ef-104">Application Gateway ondersteunt het omleiden van verkeer op basis van een gedefinieerde configuratie.</span><span class="sxs-lookup"><span data-stu-id="005ef-104">Application gateway supports the ability to redirect traffic based on a defined configuration.</span></span> <span data-ttu-id="005ef-105">Voor meer informatie over omleidingen in het algemeen gaat u naar [Application Gateway redirect overview](application-gateway-redirect-overview.md) (Overzicht van omleiden met Application Gateway).</span><span class="sxs-lookup"><span data-stu-id="005ef-105">To learn more about redirection in general visit, [Application Gateway redirect overview](application-gateway-redirect-overview.md).</span></span> <span data-ttu-id="005ef-106">Dit artikel biedt voorbeelden van HTTP-naar-HTTPS-omleidingen, padgebaseerde omleidingen, multi-site-omleidingen en omleidingen naar externe sites.</span><span class="sxs-lookup"><span data-stu-id="005ef-106">This article provides examples of HTTP to HTTPS redirection, path based redirects, multi-site redirects, and redirects to external sites.</span></span>

## <a name="http-to-https-redirect-on-an-existing-application-gateway"></a><span data-ttu-id="005ef-107">HTTP-naar-HTTPS-omleiding in een bestaande toepassingsgateway</span><span class="sxs-lookup"><span data-stu-id="005ef-107">HTTP to HTTPS redirect on an existing application gateway</span></span>

<span data-ttu-id="005ef-108">In het volgende voorbeeld wordt er een nieuwe HTTP-listener gemaakt op poort 80 om aanvragen om te leiden naar de bestaande listener op poort 443.</span><span class="sxs-lookup"><span data-stu-id="005ef-108">The following example creates a new HTTP listener on port 80 to redirect requests to the existing listener on port 443.</span></span>
...
<span data-ttu-id="005ef-109">Nadat de toepassingsgateway is bijgewerkt, test u de nieuwe listener op poort 80.</span><span class="sxs-lookup"><span data-stu-id="005ef-109">Once the application gateway is updated, test the new listener on port 80.</span></span>  <span data-ttu-id="005ef-110">In het volgende voorbeeld ziet u hoe u met behulp van `curl` de omleiding kunt testen.</span><span class="sxs-lookup"><span data-stu-id="005ef-110">The following example shows using `curl` test the redirection.</span></span>
...
## <a name="path-based-redirect"></a><span data-ttu-id="005ef-111">Padgebaseerde omleiding</span><span class="sxs-lookup"><span data-stu-id="005ef-111">Path based redirect</span></span>

<span data-ttu-id="005ef-112">In het volgende voorbeeld worden een nieuwe listener en een padregel gemaakt in een bestaande toepassingsgateway die alleen het verkeer omleidt dat voldoet aan het specifieke URL-pad.</span><span class="sxs-lookup"><span data-stu-id="005ef-112">The following example creates a new listener and a path based rule on an existing application gateway that redirects only traffic meeting the specific url path.</span></span>
...
## <a name="multi-site-redirect"></a><span data-ttu-id="005ef-113">Multi-site-omleiding</span><span class="sxs-lookup"><span data-stu-id="005ef-113">Multi-site redirect</span></span>

<span data-ttu-id="005ef-114">In het volgende voorbeeld wordt er een nieuwe toepassingsgateway gemaakt met twee multi-site-listeners op poort 80.</span><span class="sxs-lookup"><span data-stu-id="005ef-114">The following example creates a new application gateway with 2 multi-site listeners on port 80.</span></span> <span data-ttu-id="005ef-115">De listeners zijn voor adatum.com en adatum.org.</span><span class="sxs-lookup"><span data-stu-id="005ef-115">The listeners are for adatum.com and adatum.org.</span></span> <span data-ttu-id="005ef-116">Er wordt een omleidingsregel gemaakt om verkeer van adatum.org naar adatum.com om te leiden.</span><span class="sxs-lookup"><span data-stu-id="005ef-116">A redirect rule is created to redirect traffic from adatum.org to adatum.com.</span></span> <span data-ttu-id="005ef-117">Aanvullende configuratie is vereist voor het configureren van CNAME-aliassen met het openbare IP-adres van de toepassingsgateway. Raadpleeg [Een domein delegeren naar Azure DNS](../dns/dns-delegate-domain-azure-dns.md) voor meer informatie over het delegeren van uw domein naar Azure DNS en het maken van CNAME-records voor uw domeinbezoek.</span><span class="sxs-lookup"><span data-stu-id="005ef-117">Additional configuration is required for configuring CNAME aliases to the application gateway public IP address, for more information on delegating your domain to Azure DNS and creating CNAME records for your domain visit, [Delegate a domain to Azure DNS](../dns/dns-delegate-domain-azure-dns.md).</span></span>
...
## <a name="redirect-to-an-external-site"></a><span data-ttu-id="005ef-118">Omleiden naar een externe site</span><span class="sxs-lookup"><span data-stu-id="005ef-118">Redirect to an external site</span></span>

<span data-ttu-id="005ef-119">In het volgende voorbeeld wordt verkeer dat de toepassingsgateway binnenkomt, omgeleid naar een externe website (bing.com).</span><span class="sxs-lookup"><span data-stu-id="005ef-119">The following example redirects traffic coming into the application gateway to an external website (bing.com).</span></span> <span data-ttu-id="005ef-120">Wanneer u de switch `TargetUrl` gebruikt, zijn de switches `IncludePath` en `IncludeQuery` niet toegestaan.</span><span class="sxs-lookup"><span data-stu-id="005ef-120">When using the `TargetUrl` switch the `IncludePath` and `IncludeQuery` switches are not allowed.</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="005ef-121">Volgende stappen</span><span class="sxs-lookup"><span data-stu-id="005ef-121">Next steps</span></span>

<span data-ttu-id="005ef-122">[Configure end to end SSL with application gateway using PowerShell](application-gateway-end-to-end-ssl-powershell.md) (End-to-end-SSL configureren met toepassingsgateway met behulp van PowerShell) bekijken</span><span class="sxs-lookup"><span data-stu-id="005ef-122">Visit [configure end to end SSL with application gateway using PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span></span>