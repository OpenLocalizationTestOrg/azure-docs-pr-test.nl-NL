...
In dit artikel helpt u begrijpen pijplijnen en activiteiten in Azure Data Factory en ze gebruiken om end-to-end gegevensgestuurde werkstromen voor de verplaatsing van gegevens en scenario's voor gegevensverwerking samen te stellen.  
...
Een gegevensfactory kan één of meer pijplijnen hebben. Een pijplijn is een logische groepering van activiteiten die samen een taak uitvoeren. De activiteiten in een pijplijn definiëren acties uitvoeren op uw gegevens. U kunt bijvoorbeeld een kopieeractiviteit gebruiken om gegevens van een on-premises SQL-server naar Azure Blob Storage te kopiëren. Vervolgens kunt u een componentactiviteit gebruiken waarmee een Hive-script op een Azure HDInsight-cluster wordt uitgevoerd om gegevens uit Blob Storage te verwerken/transformeren en uitvoergegevens te produceren. Gebruik tot slot een tweede kopieeractiviteit voor het kopiëren van de uitvoergegevens naar een Azure SQL Data Warehouse boven op welke business intelligence (BI) reporting oplossingen zijn gebouwd. 

Een activiteit kan duren voordat de invoer van nul of meer [gegevenssets](data-factory-create-datasets.md) en een of meer uitvoer produceert [gegevenssets](data-factory-create-datasets.md). Het volgende diagram toont de relatie tussen de pipeline, activiteit en gegevensset in Gegevensfactory: 
...
Een invoergegevensset vertegenwoordigt de invoer voor een activiteit in de pijplijn en een uitvoergegevensset vertegenwoordigt de uitvoer voor de activiteit. Gegevenssets identificeren gegevens binnen andere gegevensarchieven, zoals tabellen, bestanden, mappen en documenten. U kunt een gegevensset voor activiteiten in een pijplijn gebruiken. Een dataset kan bijvoorbeeld een gegevensset invoer/uitvoer van een Kopieeractiviteit of een HDInsightHive-activiteit. Zie voor meer informatie over gegevenssets [gegevenssets in Azure Data Factory](data-factory-create-datasets.md) artikel.
...
## <a name="pipeline-json"></a>Pijplijn-JSON
Laat het ons Kijk eens op hoe een pijplijn is gedefinieerd in JSON-indeling. De algemene structuur voor een pijplijn ziet er als volgt uit:
...
| --- | --- | --- |
| naam |Naam van de pijplijn. Geef een naam die staat voor de actie die de pijplijn uitvoert. <br/><ul><li>Maximum aantal tekens: 260</li><li>Moet beginnen met een letter getal of een onderstrepingsteken (_)</li><li>Volgende tekens zijn niet toegestaan: '. ', '+ ','?', '/', '< ',' >', ' * ', "%", "&", ":","\\'</li></ul> |Ja |
| Beschrijving | Geef de tekst met een beschrijving van wat de pijplijn wordt gebruikt. |Ja |
| activiteiten | De **activiteiten** sectie kan een of meer activiteiten die zijn gedefinieerd binnen deze hebben. Zie de volgende sectie voor meer informatie over de activiteiten JSON-element. | Ja |  
...
## <a name="activity-json"></a>JSON van de activiteit
De **activiteiten** sectie kan een of meer activiteiten die zijn gedefinieerd binnen deze hebben. Elke activiteit heeft de volgende structuur op het hoogste niveau:
...
Volgende tabel beschrijft de eigenschappen in de activiteits-JSON-definitie:

| Label | Beschrijving | Vereist |
| --- | --- | --- |
| naam | De naam van de activiteit. Geef een naam die staat voor de actie die de activiteit wordt uitgevoerd. <br/><ul><li>Maximum aantal tekens: 260</li><li>Moet beginnen met een letter getal of een onderstrepingsteken (_)</li><li>Volgende tekens zijn niet toegestaan: '. ', '+ ','?', '/', '< ',' >', ' * ', "%", "&", ":","\\'</li></ul> |Ja |
| Beschrijving | Beschrijving van wat de activiteit of wordt gebruikt voor |Ja |
| type | Type van de activiteit. Zie de [activiteiten voor gegevensverplaatsing](#data-movement-activities) en [activiteiten voor gegevenstransformatie](#data-transformation-activities) secties voor verschillende soorten activiteiten. |Ja |
...
| linkedServiceName |Naam van de gekoppelde service die wordt gebruikt door de activiteit. <br/><br/>Een activiteit kan vereisen dat u opgeeft dat de gekoppelde service die is gekoppeld aan de vereiste compute-omgeving. |Ja voor HDInsight-activiteit en Azure Machine Learning-score-activiteit <br/><br/>Niet vereist voor alle andere |
| typeProperties |Eigenschappen in de **typeProperties** sectie, is afhankelijk van het type van de activiteit. Klik op koppelingen naar de activiteit in de vorige sectie overzicht van type-eigenschappen voor een activiteit. | Nee |
| Beleid |Beleidsregels die van invloed zijn op de runtime-gedrag van de activiteit. Als deze niet is opgegeven, worden de standaardbeleidsregels gebruikt. |Nee |
...
## <a name="sample-copy-pipeline"></a>Voorbeeldpijplijn kopiëren
In het volgende voorbeeldpijplijn is een activiteit van het type **kopie** in de **activiteiten** sectie. In dit voorbeeld wordt de [kopieeractiviteit](data-factory-data-movement-activities.md) kopieert gegevens van een Azure Blob-opslag naar een Azure SQL database. 
...
* De invoer voor de activiteit is ingesteld op **InputDataset** en de uitvoer voor de activiteit is ingesteld op **OutputDataset**. Zie [gegevenssets](data-factory-create-datasets.md) artikel voor het definiëren van gegevenssets in JSON. 
...
* In het gedeelte activiteiten is er slechts één activiteit waarvan **type** is ingesteld op **HDInsightHive**.
...
De **typeProperties** sectie verschilt voor elke transformatieactiviteit. Voor meer informatie over ondersteunde voor een transformatieactiviteit type-eigenschappen, klikt u op de transformatieactiviteit in de [activiteiten voor gegevenstransformatie](#data-transformation-activities) tabel. 
...
De vorige twee steekproeven pijplijnen zich slechts één activiteit bevinden. U kunt meer dan één activiteit in een pijplijn hebben.  
...
- [Een pijplijn met een transformatieactiviteit gegevens bouwen](data-factory-build-your-first-pipeline.md)
