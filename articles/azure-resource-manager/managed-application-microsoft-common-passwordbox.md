...
Een besturingselement dat kan worden gebruikt tooprovide en bevestig een wachtwoord. Gebruik van dit element wanneer [maken van een Azure-toepassing beheerd](managed-application-publishing.md).
...
- Dit element ondersteunt geen Hallo `defaultValue` eigenschap.
- Implementatiedetails van de voor `constraints`, Zie [Microsoft.Common.TextBox](managed-application-microsoft-common-textbox.md).
- Als `options.hideConfirmation` te is ingesteld,**true**, tweede tekstvak voor het bevestigen van het wachtwoord van gebruiker Hallo Hallo wordt verborgen. de standaardwaarde Hallo is **false**.
