...
## <a name="sql-server-dmvs-applicable-toosql-data-warehouse"></a>SQL Server-DMV's van toepassing tooSQL Data Warehouse
Hallo volgende DMV's zijn van toepassing tooSQL Data Warehouse, maar moeten worden uitgevoerd door verbinding te maken toohello **master** database.
...
SQL Data Warehouse wordt veel van de weergaven van de dynamische Beheerweergave Hallo SQL Server (DMV's). Deze weergaven wanneer een query uitgevoerd in SQL Data Warehouse zijn Hallo status van de SQL-Databases die zijn uitgevoerd op Hallo distributies rapportage.

Omdat SQL Data Warehouse is gebaseerd op de MPP-technologie van Microsoft, Hallo zowel SQL Data Warehouse en Analytics Platform System Parallel Data Warehouse (PDW) gebruik dezelfde systeemweergaven.

Daarom elk van deze DMV heeft een specifieke kolom pdw_node_id aangeroepen. Dit is Hallo Hallo-id voor Hallo rekenknooppunt. In PDW is Hallo rekenknooppunt een sterkere concept voor Hallo-architectuur. In SQL Data Warehouse Hallo architectuur meer sterk afhankelijk Hallo-distributies.

> [!NOTE]
> toouse deze weergeven, 'pdw_nodes_' invoegen in Hallo-naam, zoals wordt weergegeven in de volgende tabel Hallo.
> 
> 

| De naam van de DMV in SQL Data Warehouse | Koppeling tooSQL Server Transact-SQL-onderwerp op MSDN |
|:--- |:--- |
