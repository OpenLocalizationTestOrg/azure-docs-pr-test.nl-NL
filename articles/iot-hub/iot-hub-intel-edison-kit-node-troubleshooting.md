...
Zodra Hallo foutopsporing engine gereed is, moet u kunnen toosee ```Debugger listening on port 5858``` van Hallo console-uitvoer.

### <a name="configure-vs-code-tooconnect-toohello-remote-device"></a>VS-Code tooconnect toohello externe apparaten configureren

Open Hallo **Debug** deelvenster vanuit Hallo linkerkant.

Klik op Hallo groen **foutopsporing starten** knop (F5). VS Code zouden worden geopend een **launch.json** bestand, die u moet tooupdate.

Update Hallo **launch.json** bestandsinhoud Hello te volgen, worden vervangen door `[device hostname or IP address]` met Hallo daadwerkelijk apparaat IP-adres of hostnaam.  
...
Klik op Hallo groen **foutopsporing starten** (F5) knop en profiteer van foutopsporing.

U kunt lezen [JavaScript in VS-Code](https://code.visualstudio.com/docs/languages/javascript#_debugging) toolearn meer informatie over Hallo foutopsporing.
...
Ga terug te[aan de slag met Intel Edison (Node.js)](iot-hub-intel-edison-kit-node-get-started.md)
