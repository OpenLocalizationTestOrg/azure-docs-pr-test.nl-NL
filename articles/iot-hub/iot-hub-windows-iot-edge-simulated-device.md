...
---
# <a name="use-azure-iot-edge-toosend-device-to-cloud-messages-with-a-simulated-device-windows"></a>Gebruik Azure IoT rand toosend apparaat-naar-cloud-berichten met een gesimuleerd apparaat (Windows)
...
Hallo **build.cmd** script genereert de uitvoer in Hallo **bouwen** map in het lokale exemplaar van Hallo **iot-edge** opslagplaats. Deze uitvoer bevat Hallo vier IoT Edge-modules die in dit voorbeeld worden gebruikt.

Hallo build script plaatsen de:

* **Logger.dll** in Hallo **bouwen\\modules\\berichtenlogboek\\Debug** map.
* **iothub.dll** in Hallo **bouwen\\modules\\iothub\\Debug** map.
* **identiteit\_map.dll** in Hallo **bouwen\\modules\\identitymap\\Debug** map.
* **gesimuleerde\_device.dll** in Hallo **bouwen\\modules\\gesimuleerde\_apparaat\\Debug** map.

Gebruik deze paden voor Hallo **pad module** waarden zoals weergegeven in de volgende JSON-instellingenbestand Hallo:

Hallo gesimuleerde\_apparaat\_cloud\_uploaden\_Hallo pad tooa JSON-configuratiebestand als een opdrachtregelargument duurt dit voorbeeld. Hallo volgende voorbeeld van JSON-bestand vindt u in Hallo SDK opslagplaats op **voorbeelden\\gesimuleerde\_apparaat\_cloud\_uploaden\_voorbeeld\\src\\ gesimuleerde\_apparaat\_cloud\_uploaden\_voorbeeld\_win.json**. Deze configuratie bestand werkt zoals tenzij u Hallo wijzigen build script tooplace Hallo IoT rand modules of uitvoerbare bestanden in niet-standaardlocaties steekproef.

> [!NOTE]
> Hallo module paden zijn relatieve toohello directory waar Hallo gesimuleerde\_apparaat\_cloud\_uploaden\_sample.exe zich bevindt. Hallo JSON voorbeeldconfiguratiebestand standaard toowriting too'deviceCloudUploadGatewaylog.log' in de huidige werkmap.

Open Hallo-bestand in een teksteditor **voorbeelden\\gesimuleerde\_apparaat\_cloud\_uploaden\_voorbeeld\\src\\gesimuleerde\_apparaat \_cloud\_uploaden\_win.json** in het lokale exemplaar van Hallo **iot-edge** opslagplaats. Dit bestand configureert Hallo IoT rand modules in Hallo voorbeeld gateway:
...
* Hallo **BLE1** en **BLE2** modules Hallo gesimuleerde apparaten zijn. Houd er rekening mee hoe Hallo module MAC-adressen overeenkomen met Hallo-adressen in Hallo **toewijzing** module.
* Hallo **berichtenlogboek** module Logboeken uw gateway activiteit tooa-bestand.
* Hallo **pad module** waarden weergegeven in het volgende voorbeeld Hallo zijn relatieve toohello directory waar Hallo gesimuleerde\_apparaat\_cloud\_uploaden\_sample.exe zich bevindt.
...
1. Navigeer via een opdrachtprompt toohello **bouwen** map in het lokale exemplaar van Hallo **iot-edge** opslagplaats.
...
toogain een uitgebreidere kennis van IoT-rand en experiment met enkele codevoorbeelden, gaat u naar de volgende Hallo developer zelfstudies en bronnen:
