...
Een token ID is een vorm van het beveiligingstoken dat uw app van de Azure AD B2C ontvangt `authorize` en `token` eindpunten. ID-tokens worden weergegeven als [JWTs](#types-of-tokens), en bevatten claims die u gebruiken kunt om gebruikers in uw app te identificeren. Wanneer de ID-tokens zijn verkregen van de `authorize` eindpunt, ze worden vaak gebruikt voor het aanmelden van gebruikers tot webtoepassingen. Wanneer de ID-tokens zijn verkregen van de `token` eindpunt, ze kunnen worden verzonden in HTTP-aanvragen tijdens de communicatie tussen de twee onderdelen van de dezelfde toepassing of service. Zoals u dat wilt, kunt u de claims in een token ID. Ze worden vaak gebruikt om gegevens van het account weer te geven of om access control beslissingen te nemen in een app.  
