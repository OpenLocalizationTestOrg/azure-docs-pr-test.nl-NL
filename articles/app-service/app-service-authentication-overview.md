...
Als u meteen aan de slag wilt, Zie een van de volgende zelfstudies:
...
App Service kunt uw toepassing beveiligen met behulp van Azure Active Directory voor scenario's met service-naar-service. De aanroepende toepassing moet alleen een Azure Active Directory service principal verificatietoken die wordt verkregen door de client-ID en geheime van Azure Active Directory-client bieden. Een voorbeeld van dit scenario die gebruikmaakt van ASP.NET-API-apps in de zelfstudie wordt uitgelegd [principal verificatie van de Service voor API-Apps][apia-service].
...
* Alleen geverifieerde aanvragen toestaan om uw toepassing bereiken.
    ...
    Als u een browser een anonieme aanvraag ontvangt, omleidt App-Service naar een pagina voor de id-provider die u kiest, zodat gebruikers zich kunnen aanmelden. Als de aanvraag afkomstig van een mobiel apparaat is, het geretourneerde antwoord is een HTTP *401 niet geautoriseerd* antwoord.
...
De volgende zelfstudies laten zien hoe u verificatie toevoegen aan een webtoepassing:
...
De volgende zelfstudies laten zien hoe uw API-apps beveiligen:
