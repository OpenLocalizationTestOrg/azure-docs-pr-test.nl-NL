...
* De lokale cache is alleen-lezen. Echter, eventuele wijzigingen worden verwijderd wanneer de web-app kunt u virtuele machines verplaatsen of opnieuw wordt opgestart. U moet de lokale Cache niet gebruiken voor apps die bedrijfskritieke gegevens opslaan in de store.
...
* D:\Home verwijst naar de lokale cache. D:\Local blijven die verwijst naar de tijdelijke specifieke VM-opslag.
...
Het is raadzaam dat u de lokale Cache in combinatie met gebruiken de [Faseringsomgevingen](../app-service-web/web-sites-staged-publishing.md) functie.
...
* Een tijdelijke instellingen omvatten de naam en een tijdelijke in een sleuf. Dus wanneer de Staging-sleuf opgehaald gewisseld naar de productie, worden overgenomen door de instellingen van de lokale Cache-app. De zojuist gewisseld productiesite wordt uitgevoerd aan de lokale cache na een paar minuten en wordt opgewarmd als onderdeel van sleuf opwarmtijd na wisselen. Wanneer de wisseling van sleuven voltooid is, wordt dus de productiesite uitgevoerd op basis van de lokale cache.
...
Als u de functie van de lokale Cache met Faseringsomgevingen, de wisselbewerking kan pas worden voltooid lokale Cache is opgewarmd. U kunt de omgevingsvariabele voor worker-proces controleren om te controleren of uw site wordt uitgevoerd op basis van de lokale Cache, `WEBSITE_LOCALCACHE_READY`. Volg de instructies op de [worker proces omgevingsvariabele](https://github.com/projectkudu/kudu/wiki/Process-Threads-list-and-minidump-gcdump-diagsession#process-environment-variable) pagina voor toegang tot de omgevingsvariabele worker-proces op meerdere exemplaren.  
...
Als onderdeel van de stap die de inhoud van de opslag kopieert worden een map met de naam opslagplaats uitgesloten. Dit helpt met scenario's waar de inhoud van uw site een resourcebeheerbibliotheek die niet nodig in dagelijkse bewerking van de web-app zijn kan bevatten. 
