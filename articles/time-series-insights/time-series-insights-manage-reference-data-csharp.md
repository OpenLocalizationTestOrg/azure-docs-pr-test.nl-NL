...
Dit C#-voorbeeld laat zien hoe toomanage verwijzen naar gegevens voor een Azure Time Series Insights-omgeving.
Voordat u actieve Hallo-voorbeeld moet Hallo stappen te volgen zijn voltooid.
1. Een verwijzing gegevensset is gemaakt met behulp van [in dit artikel](time-series-insights-add-reference-data-set.md).
2. Hallo-toegangstoken gebruikt bij het uitvoeren van de toepassing hello is verkregen via hello Azure Active Directory-API. Dit token moet worden doorgegeven in Hallo `Authorization` koptekst van elke Query API-aanvraag. Zie voor het instellen van niet-interactieve toepassingen, Hallo [verificatie en autorisatie](time-series-insights-authentication-and-authorization.md) artikel.
3. Alle Hallo-constanten zijn gedefinieerd op Hallo begin van Hallo voorbeeld zijn correct ingesteld.
