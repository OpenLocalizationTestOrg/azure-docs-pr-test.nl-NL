...
---
# <a name="query-data-from-hello-azure-time-series-insights-environment-using-c"></a>Opvragen van gegevens uit hello Azure Time Series Insights omgeving met C#

Dit C#-voorbeeld laat zien hoe tooquery gegevens uit Azure Time Series Insights omgeving Hallo.
Hallo-voorbeeld ziet u enkele eenvoudige voorbeelden van Query API-gebruik:
1. Als een stap ter voorbereiding Hallo toegangstoken via hello Azure Active Directory-API aan te schaffen. Geeft dit token in Hallo `Authorization` koptekst van elke Query API-aanvraag. Zie voor het instellen van niet-interactieve toepassingen, [verificatie en autorisatie](time-series-insights-authentication-and-authorization.md). Controleer ook alle Hallo-constanten zijn gedefinieerd op Hallo begin van Hallo voorbeeld correct zijn ingesteld.
2. Hallo-lijst van omgevingen die gebruiker Hallo heeft toegang toois verkregen. Een van de Hallo omgevingen wordt opgehaald als Hallo-omgeving van belang en verdere gegevens voor deze omgeving wordt opgevraagd.
3. Als een voorbeeld van HTTPS-aanvraag is voor Hallo-omgeving van belang beschikbaarheidsgegevens aangevraagd.
4. Als een voorbeeld van de webaanvraag voor de socket is voor Hallo-omgeving van belang statistische gegevens over gebeurtenis aangevraagd. Gegevens is voor Hallo hele beschikbaarheid tijdsbereik aangevraagd.
...
Zie voor volledige Query API-verwijzing hello, Hallo [Query API](/rest/api/time-series-insights/time-series-insights-reference-queryapi) document.
